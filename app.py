    ...
    for poste in vp:
        ...
        for kpi in all_kpis:
            ...
            if len(hist) >= 2:
                ...
                if abs(delta) <= 0.5: trend, ticon, tcol = "Stable", "\u27a1\ufe0f", "#718096"
                elif (delta > 0.5 and not lb) or (delta < -0.5 and lb): trend, ticon, tcol = "Amelioration", "\U0001f4c8", "#38a169"
                else: trend, ticon, tcol = "Degradation", "\U0001f4c9", "#e53e3e"
            else:
                prev, delta = None, None
                trend, ticon, tcol = "Nouvelle", "\U0001f195", "#805ad5"
            met = (current <= target) if lb else (current >= target)
            rows.append({"Poste de Travail": poste, "KPI": kpi, "Valeur Actuelle": current, "Valeur Precedente": prev if prev is not None else "N/A", "Delta": delta if delta is not None else "N/A", "Delta_Raw": delta, "Tendance": "%s %s" % (ticon, trend), "Tendance_Color": tcol, "Cible": target, "Atteint": met, "LB": lb})
...
def generate_recommendations(ckdf, cible, vp, kpi_history, all_kpis):
    ...
    for poste in vp:
        ...
        for kpi in all_kpis:
            ...
            if worsening: prio, pcol, psort = "Critique", "#e53e3e", 0
            elif gap >= 30: prio, pcol, psort = "Critique", "#e53e3e", 0
            elif gap >= 15: prio, pcol, psort = "Elevee", "#ed8936", 1
            elif gap >= 5: prio, pcol, psort = "Moyenne", "#ecc94b", 2
            else: prio, pcol, psort = "Faible", "#48bb78", 3
            ...
            recs.append({"Priorite": prio, "Priorite_Color": pcol, "Priorite_Sort": psort, "Categorie": cat, "Poste": poste, "KPI": kpi, "Actuel": round(val, 1), "Cible": target, "Ecart": gap, "Delta": delta if delta is not None else "N/A", "Recommandation": msg})
...
def analyze_backlog_auto(dfp, avf, vp):
    ...
    total_ot = len(dfp)
    if total_ot == 0:
        findings.append({"Type": "Information", "Priorite": "Info", "Message": "Aucun OT dans les filtres selectionnes", "Detail": "Modifier les filtres."})
        return findings
    if "Statut OT" in dfp.columns:
        sc = dfp["Statut OT"].value_counts()
        for st, cnt in sc.items():
            pct = round(cnt / total_ot * 100, 1)
            findings.append({"Type": "OT - Repartition", "Priorite": "Info", "Message": "%s: %d OT (%.1f%%)" % (st, cnt, pct), "Detail": "Sur %d OT, %d en statut %s." % (total_ot, cnt, st)})
    for phase, col_a, sf in [("Preparation", "ap", dfp["Statut OT"] == "CRÉÉ"), ("Planification", "alp", (dfp["Statut OT"] == "LANC") & (dfp["Contient SOPL"] == 0)), ("Execution", "aex", (dfp["Statut OT"] == "LANC") & (dfp["Contient SOPL"] == 1))]:
        ...
        if len(old) > 0:
            ...
            findings.append({"Type": "OT - %s >3 mois" % phase, "Priorite": "Critique" if len(old) > 10 else "Elevee", "Message": "%d OT en %s depuis >3 mois" % (len(old), phase.lower()), "Detail": "Postes: %s." % pl})
    if avf is not None and not avf.empty:
        findings.append({"Type": "AVIS - Volume", "Priorite": "Info", "Message": "%d avis sans OT associe" % len(avf), "Detail": "Ces avis necessitent la creation d'OT."})
    return findings

def html_evolution_table(rows):
    h = '<table class="tw evt"><thead><tr><th>Poste</th><th>KPI</th><th>Actuel</th><th>Precedent</th><th>Delta</th><th>Tendance</th><th>Cible</th><th>Statut</th></tr></thead><tbody>'
    for r in rows:
        if r["Delta"] != "N/A":
...
def html_rec_cards(recs):
    ...
    h, cls_map = '', {0: "critique", 1: "elevee", 2: "moyenne", 3: "faible"}
    for r in recs:
...
def html_findings(findings):
    ...
    h, pcol = '', {"Critique": "#e53e3e", "Elevee": "#ed8936", "Moyenne": "#ecc94b", "Info": "#718096"}
    for f in findings:
...
def create_analyse_excel(prep_tbl, plan_tbl, exec_tbl, evol_df, recs_df, findings_df):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        for sn in writer.sheets:
            ws = writer.sheets[sn]
            for cell in ws[1]: cell.font = Font(bold=True, color="FFFFFF", size=10); cell.fill = PatternFill(start_color="1e3a5f", end_color="1e3a5f", fill_type="solid"); cell.alignment = Alignment(horizontal='center')
            for row in ws.iter_rows(min_row=2):
                for cell in row: cell.border = Border(left=Side(style='thin', color='d0d0d0'), right=Side(style='thin', color='d0d0d0'), top=Side(style='thin', color='d0d0d0'), bottom=Side(style='thin', color='d0d0d0'))
    output.seek(0)
    return output

def contient_mot(t, lm): return any(m in str(t) for l in lm for m in l.split())
def cat_age(a): return "Inconnu" if pd.isna(a) else ("<1 mois" if a <= 1 else (">3 mois" if a >= 3 else "1 mois < <3 mois"))
...
def gscore(k, a, t):
    ...
    if k in ["OT LANC ESTIME", "Backlog préparation caractérisé", "Backlog planification caractérisé", "OT CONFIME", "OT_COR_EGAL", "appel avis approuvé"]: return 1 if a >= 95 else 0
    return 0
...
def calc_kpis(df_i, av_i, now, posts):
    ...
    df["Backlog preparation"] = np.where(df["Statut utilisateur"].apply(lambda x: contient_mot(x, ["CRPR ATPD", "CRPR ATMR", "CRPR ATER", "CRPR ATRS", "CRPR ATMO", "ATPD", "ATMR", "ATER", "ATRS", "ATMO"])), "CARACTERISE", "NON CARACTERISE")
    df["Backlog planification"] = np.where(df["Statut utilisateur"].apply(lambda x: contient_mot(x, ["ATPL ATEI", "ATPL ATAL", "ATPL ATER", "ATPL AGAR", "ATPL ATHS", "ATEI", "ATAL", "ATAS", "AGAR", "ATHS"])), "CARACTERISE", "NON CARACTERISE")
    ...
    for dc, am, ac in [('Créé le', "amp", "ap"), ('Date de début planifiée', "amlp", "alp"), ('Date de début planifiée', "amex", "aex")]:
        ...
        else: df[am], df[ac] = np.nan, "Inconnu"
...
def html_synth(kpi_list, actuals, targets, act_map, accent):
    ...
    for k in kpi_list:
        ...
        sbg, sclr, scbg = ("#c6efce", "#006100", accent) if met else ("#ffc7ce", "#9c0006", "#e53e3e")
...
def html_classement(scores, accent, threshold=90):
    ...
    t5, b5 = [x for x in sp if x[1] >= threshold][:5], [x for x in sp if x[1] < threshold][-5:]
...
def html_kpi_bars(kpi_list, actuals, targets, title, color_ok, color_fail, kpi_history=None):
    ...
    for k in kpi_list:
        av, tv = actuals.get(k, 0), targets.get(k, 100)
        met, bw, bg = (av <= tv if is_lb(k) else av >= tv), min(max(av, 0), 100), (color_ok if (av <= tv if is_lb(k) else av >= tv) else color_fail)
        sp = ""
        if kpi_history:
...
def html_grouped_bars(posts, pscores, qscores, title, threshold=90):
    h = '<div class="ca"><div class="ct" style="color:#1e3a5f">%s</div><div class="gbr-legend"><span><i style="background:linear-gradient(90deg,#2b6cb0,#4299e1)"></i> Performance</span><span><i style="background:linear-gradient(90deg,#276749,#48bb78)"></i> Qualite</span><span><i style="background:#e53e3e;width:2px;height:18px"></i> Cible %.0f%%</span></div>' % (title, threshold)
    for p in sorted(posts, key=lambda x: (pscores.get(x, 0) + qscores.get(x, 0)) / 2, reverse=True):
        pv, qv, pw, qw = pscores.get(p, 0), qscores.get(p, 0), min(max(pscores.get(p, 0), 0), 100), min(max(qscores.get(p, 0), 0), 100)
...
def main():
    ...
    with st.sidebar:
        ...
        ot_f = av_f = None
        apm, df_dt = [], datetime.now().strftime("%d/%m/%Y")
    ...
    try:
        ...
        if not apm and "Poste travail princ." in raw_ot.columns:
        ...
        sp_filtered = apm if ("All" in sp or not sp) else sp
        sa_filtered = ["All"] if ("All" in sa or not sa) else sa
        sd_filtered = ["All"] if ("All" in sd or not sd) else sd
        ...
        def mf(poste):
            ...
            if sp_filtered and p not in [str(x).upper() for x in sp_filtered]: return False
            return True
        ...
        if not vp:
            st.warning("Aucun poste trouvé avec ces filtres.")
            st.stop()

        now = datetime.now()
        results = calc_kpis(df_filtered, av_filtered, now, vp)
        dfp, ckdf, avf = results['dfp'], results['ckdf'], results.get('avf', pd.DataFrame())
        
        pk = ["TAUX_REALISATION_CORRECTIF/PT", "OT préparation <1 mois", "OT préparation >3 mois", "OT préparation 1mois< <3mois", "OT planification <1 mois", "OT planification >3 mois", "OT planification 1mois< <3mois", "OT exécution <1 mois", "OT exécution >3 mois", "OT exécution 1mois< <3mois"]
        qk = ["appel avis approuvé", "OT LANC ESTIME", "Backlog préparation caractérisé", "Backlog planification caractérisé", "OT CONFIME", "OT_COR_EGAL"]
        all_kpis = pk + qk
        ...
        pscores = {poste: (sum(gscore(k, ckdf.loc[poste, k], cible.get(k, 100)) for k in pk if k in ckdf.columns and pd.notna(ckdf.loc[poste, k])) / len(pk)) * 100 for poste in vp if poste in ckdf.index and pk}
        qscores = {poste: (sum(gscore(k, ckdf.loc[poste, k], cible.get(k, 100)) for k in qk if k in ckdf.columns and pd.notna(ckdf.loc[poste, k])) / len(qk)) * 100 for poste in vp if poste in ckdf.index and qk}
        pa, qa = {k: round(float(ckdf[k].mean()), 2) for k in pk if k in ckdf.columns}, {k: round(float(ckdf[k].mean()), 2) for k in qk if k in ckdf.columns}
        p_global, q_global = round(float(np.mean(list(pscores.values()))) if pscores else 0, 1), round(float(np.mean(list(qscores.values()))) if qscores else 0, 1)
        ...
        nb_na = sum(1 for k in all_kpis if k in pa and ((is_lb(k) and pa[k] > cible.get(k, 100)) or (not is_lb(k) and pa[k] < cible.get(k, 100)))) + sum(1 for k in all_kpis if k in qa and ((is_lb(k) and qa[k] > cible.get(k, 100)) or (not is_lb(k) and qa[k] < cible.get(k, 100))))
        st.markdown('<div class="cr"><div class="cc c1"><div class="cv">%.1f%%</div><div class="cl">Score Performance</div></div><div class="cc c2"><div class="cv">%.1f%%</div><div class="cl">Score Qualité</div></div><div class="cc c3"><div class="cv">%d</div><div class="cl">Postes Suivis</div></div><div class="cc c4"><div class="cv">%d</div><div class="cl">KPI Non Atteints</div></div></div>' % (p_global, q_global, len(vp), nb_na), unsafe_allow_html=True)
        ...
        with tab1:
            ...
            st.markdown(html_grouped_bars(vp, pscores, qscores, "Comparaison par Poste", 90), unsafe_allow_html=True)
        ...
        with tab5:
            st.markdown('<div class="stl ev">TABLEAU ÉVOLUTIF</div>', unsafe_allow_html=True)
        ...
        excel_data = create_analyse_excel(pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), evol_export, recs_export, findings_export)
        st.download_button("📥 Télécharger Analyse Complète (Excel)", data=excel_data.getvalue(), file_name="analyse_maintenance.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
