# -*- coding: utf-8 -*-
# VERSION V3.1 - Anomalies transposees (KPI en lignes, Postes en colonnes)
...
st.set_page_config(layout="wide", page_title="Dashboard KPI V3.1")
...
def inject_custom_css():
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .tw.atp .kpi-cell{background:#f0fff4;font-weight:700;color:#276749;white-space:normal;max-width:240px;min-width:180px;position:sticky;left:0;z-index:5;border-right:2px solid #c6f6d5}
    .tw.atq .kpi-cell{background:#ebf8ff;font-weight:700;color:#2b6cb0;white-space:normal;max-width:240px;min-width:180px;position:sticky;left:0;z-index:5;border-right:2px solid #bee3f8}
    .tw.atp tbody tr:hover .kpi-cell{background:#dcfce7!important}
    .tw.atq tbody tr:hover .kpi-cell{background:#dbeafe!important}
    ...
    .tw tbody tr:nth-child(even) .kpi-cell{background:#f0fff4}
    .tw.atq tbody tr:nth-child(even) .kpi-cell{background:#ebf8ff}
    ...
    .tr .kpi-cell{background:#e2e8f0!important;color:#1a202c!important}
    ...
    .ano-legend{display:flex;gap:16px;margin-bottom:6px;font-size:11px;font-weight:600;color:#4a5568}
    .ano-legend span{display:flex;align-items:center;gap:4px}
    .ano-legend i{display:inline-block;width:14px;height:14px;border-radius:2px;border:1px solid #e2e8f0}
...
def main():
    ...
    # ===================================================================
    # NOUVELLE FONCTION : Table anomalie TRANSPOSEE
    # Lignes = KPI, Colonnes = Postes, Cellules = Valeur / Ecart
    # ===================================================================
    def html_ano_transposed(ano_rows, kpi_list, css_class, cell_class):
        """
        Construit un tableau HTML transpose pour les anomalies.
        - Ligne 1 (thead) : Poste de travail | Poste A | Poste B | ...
        - Lignes suivantes : KPI | valeur(écart) | valeur(écart) | ...
        - Derniere ligne : Total anomalies | nb | nb | ...
        """
        # Construire la matrice KPI x Poste
        matrix = {}
        for r in ano_rows:
            if r.get("_t") == "total": continue
            kpi = r.get("KPI", "")
            poste = r.get("Poste de travail", "")
            val = r.get("Valeur", 0)
            ecart = r.get("Ecart", 0)
            cible = r.get("Cible", 0)
        ...
        # KPIs dans l'ordre de la liste, puis ceux trouves
        ordered_kpis = [k for k in kpi_list if k in matrix]
        extra_kpis = [k for k in sorted(matrix.keys()) if k not in ordered_kpis]
        ...
        # Legende couleurs
        legend = ('<div class="ano-legend">'
                  '<span><i style="background:#fed7d7"></i> Ecart &gt; 10 pts</span>'
                  '<span><i style="background:#ffeb9c"></i> Ecart 5-10 pts</span>'
                  '<span><i style="background:#feebc8"></i> Ecart &lt; 5 pts</span>'
                  '<span><i style="background:#edf2f7"></i> N/A</span>'
                  '</div>')

        h = legend
        h += '<table class="tw %s"><thead><tr>' % css_class
        h += '<th style="min-width:200px;text-align:left">KPI</th>'
        for p in all_postes:
            h += '<th style="min-width:100px">%s</th>' % p
        h += '<th style="min-width:60px">Nb Postes</th>'
        h += '</tr></thead><tbody>'

        grand_total = 0
        for kpi in all_kpis:
            h += '<tr>'
            h += '<td class="kpi-cell">%s</td>' % kpi
            row_count = 0
            for p in all_postes:
                if p in matrix[kpi]:
                    d = matrix[kpi][p]
                    ...
                    cb = d["cible"]
                    row_count += 1
                    # Coloration par gravite de l'ecart
                    try:
                        ae = abs(float(ev))
                    except:
                        ae = 0
                    if ae > 10:
                        s = "background:#fed7d7;color:#9b2c2c;font-weight:700"
                    elif ae > 5:
                        s = "background:#ffeb9c;color:#975a16;font-weight:600"
                    else:
                        s = "background:#feebc8;color:#9c4221;font-weight:600"
                    h += '<td style="%s" title="Valeur: %.1f%% | Cible: %.0f%%">%.1f<br><span style="font-size:10px;opacity:.8">(%+.1f)</span></td>' % (s, vl, cb, vl, ev)
                else:
                    h += '<td style="color:#cbd5e0">—</td>'
        ...
        # Ligne total
        h += '<tr class="tr"><td class="kpi-cell">TOTAL ANOMALIES</td>'
        for p in all_postes:
            cnt = sum(1 for kpi in all_kpis if p in matrix.get(kpi, {}))
            h += '<td style="text-align:center;font-weight:800">%d</td>' % cnt
        h += '<td style="text-align:center;font-weight:900;font-size:14px">%d</td>' % grand_total
        h += '</tr>'
        ...
        try:
            ...
            for p in posts:
                if p in ckdf.index:
                    ps=0; qs=0
                    for k in QK:
                        a=ckdf.loc[p,k] if k in ckdf.columns else 0; t=CIBLE.get(k,100); ps+=gscore(k,a,t)
                    for k in PK:
                        a=ckdf.loc[p,k] if k in ckdf.columns else 0; t=CIBLE.get(k,100); qs+=gscore(k,a,t)
                    perf_scores[p]=ps/len(QK)*100 if QK else 0; qual_scores[p]=qs/len(PK)*100 if PK else 0
                else:
                    perf_scores[p]=0; qual_scores[p]=0
            ...
            global_actuals={}
            for k in ALL_KPI:
                global_actuals[k]=round(ckdf[k].mean(),1) if k in ckdf.columns else 0
            ...
            # Colonnes pour Excel (non transposees)
            ano_p_cols=["Poste de travail","KPI","Valeur","Cible","Ecart"]
            ano_q_cols=["Poste de travail","KPI","Valeur","Cible","Ecart"]
            # Lignes total pour Excel
            ano_p_excel=ano_p_rows.copy()
            ano_p_excel.append({"Poste de travail":"Total","KPI":"","Valeur":"","Cible":"","Ecart":tot_ano_p,"_t":"total"})
            ano_q_excel=ano_q_rows.copy()
            ano_q_excel.append({"Poste de travail":"Total","KPI":"","Valeur":"","Cible":"","Ecart":tot_ano_q,"_t":"total"})

            # ---- Tables Performance & Qualite ----
            ...
            save_kpis_to_excel(prows,pcols,qrows,qcols,ano_p_excel,ano_p_cols,ano_q_excel,ano_q_cols,fichier_date)
            ...
            tab_synthese, tab_performance, tab_qualite, tab_anomalies, tab_journal, tab_classement = st.tabs([
                "📋 Synthese & Actions","⚡ Performance","🎯 Qualite","⚠️ Anomalies","📖 Journal","🏆 Classement"
            ])
            ...
            with tab_synthese:
                ...
                with col_p1:
                    fig_ot=anl_pie_chart(ot_statut_df,"Statut OT","Nombre","OT par Statut",
                ...
                with col_p2:
                    fig_av=anl_pie_chart(avis_statut_df,"Statut utilisateur","Nombre","Avis par Statut",
                        colors=["#38a169","#3182ce","#d69e2e","#e53e3e","#805ad5","#CBD5E0"])
                ...
                else: st.markdown('<div class="es">Aucun poste</div>',unsafe_allow_html=True)
                ...
                non_atteints_p=[k for k in QK if not (global_actuals.get(k,0)<=CIBLE.get(k,100) if is_lb(k) else global_actuals.get(k,0)>=CIBLE.get(k,100))]
                non_atteints_q=[k for k in PK if not (global_actuals.get(k,0)<=CIBLE.get(k,100) if is_lb(k) else global_actuals.get(k,0)>=CIBLE.get(k,100))]
                col_a1, col_a2 = st.columns(2)
                with col_a1:
                    st.markdown('<div class="ca"><div class="ct" style="color:#38a169">Actions Performance</div>',unsafe_allow_html=True)
                    if non_atteints_p: st.markdown(html_actions_table(non_atteints_p,global_actuals,CIBLE,ACT_MAP),unsafe_allow_html=True)
                    else: st.markdown('<div class="es">✅ Tous les KPI Performance atteints</div>',unsafe_allow_html=True)
                    st.markdown('</div>',unsafe_allow_html=True)
                with col_a2:
                    st.markdown('<div class="ca"><div class="ct" style="color:#3182ce">Actions Qualite</div>',unsafe_allow_html=True)
                    if non_atteints_q: st.markdown(html_actions_table(non_atteints_q,global_actuals,CIBLE,ACT_MAP),unsafe_allow_html=True)
                    else: st.markdown('<div class="es">✅ Tous les KPI Qualite atteints</div>',unsafe_allow_html=True)
                    st.markdown('</div>',unsafe_allow_html=True)
            ...
            with tab_performance:
                ...
                with st.expander("📊 Graphique detaille Performance"):
                    fig_p2=make_kpi_bar_chart_with_target(QK,global_actuals,CIBLE,"Performance — Detail KPI","#38a169","#e53e3e")
            ...
            with tab_qualite:
                ...
                with st.expander("📊 Graphique detaille Qualite"):
                    fig_q2=make_kpi_bar_chart_with_target(PK,global_actuals,CIBLE,"Qualite — Detail KPI","#3182ce","#e53e3e")
                    if fig_q2: st.plotly_chart(fig_q2,use_container_width=True)

            # ============ ONGLET 4 : ANOMALIES (TRANSPOSE) ============
            with tab_anomalies:
                ...
                if tot_ano_p == 0 and tot_ano_q == 0:
                    st.markdown('<div class="es" style="padding:30px;font-size:16px">✅ Aucune anomalie detectee — Tous les indicateurs sont atteints</div>',unsafe_allow_html=True)
                else:
                    # --- Anomalies Performance (transposées) ---
                    if tot_ano_p > 0:
                        ...
                        st.markdown(
                            html_ano_transposed(ano_p_rows, QK, "atp", "kpi-cell"),
                    ...
                    if tot_ano_q > 0:
                        ...
                        st.markdown(
                            html_ano_transposed(ano_q_rows, PK, "atq", "kpi-cell"),
                            unsafe_allow_html=True
                        )
                    else:
            ...
            with tab_journal:
                ...
                if hist_df.empty:
                    st.markdown('<div class="es">Pas assez d\'historique pour calculer les variations (minimum 2 periodes)</div>',unsafe_allow_html=True)
                else:
            ...
            with tab_classement:
                ...
                if not hist_df.empty:
                    ...
                    if not var_df.empty:
                        ...
                        with col_h1:
                            if not top_h.empty:
                                st.markdown("**🟢 Top 5 Progression**")
                                st.dataframe(top_h,use_container_width=True)
                        with col_h2:
                            if not bot_h.empty:
                                st.markdown("**🔴 Bottom 5 Regression**")
                                st.dataframe(bot_h,use_container_width=True)

        except Exception as e:
            st.error("Erreur de chargement: %s"%str(e))
            st.info("Verifiez que les fichiers ot.xlsx et avis.xlsx sont presents a la racine du projet.")
    else:
        if unf:
            ...
            st.markdown("""<div style="text-align:center;padding:60px;color:#718096"><div style="font-size:64px;margin-bottom:16px">📂</div><h2 style="color:#1e3a5f">Fichiers non trouves</h2><p>Placez les fichiers <b>ot.xlsx</b> et <b>avis.xlsx</b> a la racine du projet.</p></div>""",unsafe_allow_html=True)
