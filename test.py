...
def inject_custom_css():
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .main .block-container{max-width:100%!important;width:100%!important;padding-top:.6rem;padding-bottom:.6rem;padding-left:0.5rem!important;padding-right:0.5rem!important}
    ...
    .mh h1{color:#fff;font-size:24px;font-weight:800;margin:0;display:inline}
    ...
    if not unf or (ot_f is not None and av_f is not None):
        try:
            ...
            with tab1:
                ...
                st.markdown(html_kpi_bars(qk, pa, cible, "Progression des KPI Performance", "#38a169", "#e53e3e"), unsafe_allow_html=True)

                st.markdown('<div class="stl p">📋 Detail par poste — Performance</div>', unsafe_allow_html=True)
            ...
            with tab2:
                ...
                st.markdown(html_kpi_bars(pk, qa, cible, "Progression des KPI Qualite", "#3182ce", "#e53e3e"), unsafe_allow_html=True)

                st.markdown('<div class="stl q">📋 Detail par poste — Qualite</div>', unsafe_allow_html=True)
            ...
            # ==================== ANALYSE ====================
            with tab3:
                st.markdown('<div class="stl c">🔬 Analyses approfondies</div>', unsafe_allow_html=True)

                # Statuts OT
                if "Statut OT" in dfp.columns:
                    st.markdown('<div class="stl c">Répartition par Statut OT</div>', unsafe_allow_html=True)
                    ...
                    st.markdown(anl_html_table(stat_counts, "Pourcentage"), unsafe_allow_html=True)
                    fig = anl_pie_chart(stat_counts, "Statut", "Nombre", "Répartition par Statut OT")
                    if fig: st.plotly_chart(fig, use_container_width=True)

                # Par type d'intervention
                if "Type d'intervention" in dfp.columns:
                    st.markdown('<div class="stl c">Répartition par Type d\'intervention</div>', unsafe_allow_html=True)
                    ti_counts = dfp["Type d'intervention"].value_counts().reset_index()
                    ti_counts.columns = ["Type", "Nombre"]
                    ti_counts["Pourcentage"] = (ti_counts["Nombre"] / ti_counts["Nombre"].sum() * 100).round(1)
                    st.markdown(anl_html_table(ti_counts, "Pourcentage"), unsafe_allow_html=True)

                # Age OT par statut
                for stat_val, stat_label in [("CRÉÉ","Créés"),("LANC","Lancés")]:
                    sdf = dfp[dfp["Statut OT"]==stat_val]
                    if not sdf.empty and "amp" in sdf.columns:
                        st.markdown('<div class="stl c">Age moyen des OT %s (mois)</div>' % stat_label, unsafe_allow_html=True)
                        age_by_poste = sdf.groupby("Poste travail princ.")["amp"].agg(["mean","count"]).round(1)
                        age_by_poste.columns = ["Age moyen (mois)", "Nombre OT"]
                        age_by_poste = age_by_poste.sort_values("Age moyen (mois)", ascending=False)
                        st.markdown(anl_html_table(age_by_poste.reset_index()), unsafe_allow_html=True)

                # Top postes avec le plus d'anomalies
                if all_ano:
                    st.markdown('<div class="stl a">⚠️ Postes les plus problématiques</div>', unsafe_allow_html=True)
                    ano_df = pd.DataFrame(all_ano)
                    ano_by_poste = ano_df.groupby("Poste")["Nb"].sum().sort_values(ascending=False).reset_index()
                    ano_by_poste.columns = ["Poste", "Total Anomalies"]
                    st.markdown(anl_html_table(ano_by_poste), unsafe_allow_html=True)

                # Export
                st.markdown("---")
                export_btn(ckdf.reset_index().round(2), "kpi_detail_%s.xlsx" % fichier_date.replace("/","-"))
