...
CHANGELOG = [
    {"version":"2.4","date":"2025-06-22","changes":[
        "Tableaux de synthèse d'évolution simplifiés (ecart uniquement avec code couleur)",
        "Affichage des graphiques cote a cote avec les tableaux dans la page Backlog",
        "Ajout d'une ligne de cible (80%) sur les graphiques de scores par poste",
        "Bouton d'export en mode presentation (Slide/PDF)"
    ]},
    {"version":"2.3","date":"2025-06-21","changes":[
        "Affichage des scores globaux par Division (SF1/SF2) en haut du tableau de bord",
        "Page Backlog enrichie avec synthese Caracterise/Non Caracterise et detail par type",
        "Integration definitive des tableaux d'anomalies sous les indicateurs Perf et Qualite",
        "Correction de l'erreur KeyError 'Sens' lors de la generation de la synthese historique"
    ]}
]
...
def inject_custom_css():
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .gbr-target{position:absolute;left:80%;top:-2px;bottom:-2px;width:2px;background:#e53e3e;z-index:10;box-shadow:0 0 2px rgba(0,0,0,.5)}
...
def main():
    ...
    def show_simple_pie(piv_df, title):
        ...
        if not counts.empty:
            ...
            fig.update_layout(margin=dict(t=40,b=10,l=10,r=10),height=350,legend=dict(font_size=10,orientation="h",yanchor="bottom",y=-0.1))
    ...
    def html_grouped_bars(posts,pscores,qscores,title):
        ...
        for p in sorted(posts,key=lambda x:(pscores.get(x,0)+qscores.get(x,0))/2,reverse=True):
            ...
            h+='<div class="gbr"><div class="gbr-l">%s</div><div class="gbr-g"><div class="gbr-target"></div><div class="gbr-w"><div class="gbr-f gb-p" style="width:%s%%"></div></div><div class="gbr-v">%.1f%%</div><div class="gbr-w"><div class="gbr-f gb-q" style="width:%s%%"></div></div><div class="gbr-v">%.1f%%</div></div></div>'%(p,min(max(pv,0),100),pv,min(max(qv,0),100),qv)
        return h+'</div>'
        ...
        for poste in posts:
            ...
            for kpi in kpi_list:
                ...
                if diff != "—":
                    try:
                        d = float(diff)
                        if d > 0: clr="#c6efce" # vert
                        elif d < 0: clr="#ffc7ce" # rouge
                        else: clr="" # stable
    ...
    if not unf or (ot_f is not None and av_f is not None):
        try:
            ...
            with tabs[0]:
                st.markdown('<div class="stl p">Scores globaux par poste (Ligne rouge = Cible 80%)</div>',unsafe_allow_html=True)
            ...
            with tabs[3]:
                ...
                with c3:
                    st.markdown(html_generic_pivot(piv_carac_prep_type, "tht", "Répartition par Type de Caractérisation (Caractérisés uniquement)"),unsafe_allow_html=True)
                with c4:
                    show_simple_pie(piv_carac_prep_type, "Répartition par Type de Caractérisation")
                ...
                c7, c8 = st.columns([0.6, 0.4])
                with c7:
                    st.markdown(html_generic_pivot(piv_carac_plan_type, "tht", "Répartition par Type de Caractérisation (Caractérisés uniquement)"),unsafe_allow_html=True)
                with c8:
                    show_simple_pie(piv_carac_plan_type, "Répartition par Type de Caractérisation")
            ...
            with tabs[4]:
                ...
                if synth_qual and any(any(v.get("diff","—")!="—" for v in d.values()) for d in synth_qual.values()):
                    ...
                    st.markdown('<div class="es">Pas assez de donnees histo.pyriques pour calculer la synthese Qualite. Au moins 2 periodes sont necessaires.</div>',unsafe_allow_html=True)
                ...
                if os.path.exists(hist_filepath):
                    with open(hist_filepath, "rb") as f:
                        st.download_button("Télécharger l'historique complet des KPIs (Excel)", data=f, file_name="indicateurs_kpis_historique.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                else:
