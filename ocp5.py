...
CHANGELOG = [
    {"version":"2.5","date":"2025-06-23","changes":[
        "Ajout export Excel détaillé des anomalies OT et Avis par KPI",
        "Protection du chargement de fichiers par mot de passe (779900) et mise à jour de la date",
        "Optimisation du mode Présentation (Slide/PDF)",
        "Ligne d'objectif fixée à 90% sur le graphique des scores par poste"
    ]}
]
...
def inject_custom_css():
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .gbr-target{position:absolute;left:90%;top:-2px;bottom:-2px;width:2px;background:#e53e3e;z-index:10;box-shadow:0 0 2px rgba(0,0,0,.5)}
...
def main():
    ...
    def show_simple_pie(piv_df, title):
        ...
        if not counts.empty:
            ...
            fig.update_layout(margin=dict(t=40,b=10,l=10,r=10),height=300,legend=dict(font_size=10,orientation="h",yanchor="bottom",y=-0.1))
    ...
    if not unf or (ot_f is not None and av_f is not None):
        try:
            ...
            with tabs[0]:
                st.markdown('<div class="stl p">Scores globaux par poste (Ligne rouge = Cible 90%)</div>',unsafe_allow_html=True)
