# -*- coding: utf-8 -*-
# VERSION V3.3 - Correction KeyError 'Statut OT' via standardisation automatique des colonnes
...
st.set_page_config(layout="wide", page_title="Dashboard KPI V3.3")
...
def main():
    ...
    # ===================================================================
    # MOTEUR DE STANDARDISATION DES COLONNES
    # ===================================================================
    def std_cols(df):
        df.columns = [str(c).strip() for c in df.columns]
        mp = {
            "Statut_ot":"Statut OT","StatutOT":"Statut OT","Statut ot":"Statut OT",
            "Poste de travail princ.":"Poste travail princ.","Poste de travail princ":"Poste travail princ.",
            "N° appel pl.entret.":"Nº appel pl.entret.","Nº appel pl. entret.":"Nº appel pl.entret.",
            "Date de debut planifiee":"Date de début planifiée","Debut souhaite":"Début souhaité",
            "Total couts budgétés":"Total coûts budgétés","Total couts reels":"Total coûts réels",
            "Statut systeme":"Statut système","Debut reel":"Début réel","Fin reelle":"Fin réelle",
            "N° avis":"Avis","Nº avis":"Avis","Date de la cloture":"Date de la clôture",
            "N° ordre":"Ordre","Nº ordre":"Ordre","Numero d'ordre":"Ordre",
        }
    ...
    with st.sidebar:
        ...
        if show_filters:
            ...
            st.markdown("---")
            st.markdown("**🎯 Postes**"); sp=st.multiselect("Poste",["All"]+apm,["All"],key="sp")
    ...
    if not unf or (ot_f is not None and av_f is not None):
        try:
            ...
            # ---- STANDARDISATION AUTOMATIQUE DES COLONNES ----
            raw_ot = std_cols(raw_ot)
            raw_av = std_cols(raw_av)
            
            if "Statut OT" not in raw_ot.columns:
                if "Statut système" in raw_ot.columns:
                    raw_ot["Statut OT"] = raw_ot["Statut système"].apply(map_statut_ot)
                else:
                    raw_ot["Statut OT"] = "CRÉÉ"
            
            if "Avis" not in raw_av.columns:
                for c in raw_av.columns:
                    if "avis" in c.lower():
                        raw_av["Avis"] = raw_av[c]; break
                else:
                    raw_av["Avis"] = raw_av.index
            ...
            if not df_oms.empty and "Statut OT" in df_oms.columns:
                oms_pivot = pd.pivot_table(df_oms, index="Poste travail princ.", columns="Statut OT", values="Ordre", aggfunc="count", fill_value=0).reindex(posts, fill_value=0)
            ...
            if not df_thermo.empty and "Statut OT" in df_thermo.columns:
                thermo_pivot = pd.pivot_table(df_thermo, index="Poste travail princ.", columns="Statut OT", values="Ordre", aggfunc="count", fill_value=0).reindex(posts, fill_value=0)
            ...
            with tab_synthese:
                ...
                fig_tp=make_rank_bar_chart(top5_perf,bot5_perf,"Top 5 / Bottom 5 — Performance","#38a169","#e53e3e")
                ...
                fig_tq=make_rank_bar_chart(top5_qual,bot5_qual,"Top 5 / Bottom 5 — Qualite","#3182ce","#e53e3e")
            ...
            with tab_performance:
                ...
                with st.expander("📊 Graphique detaille"):
                    fig_p2=make_kpi_bar_chart_with_target(QK,global_actuals,CIBLE,"Performance","#38a169","#e53e3e")
            ...
            with tab_qualite:
                ...
                with st.expander("📊 Graphique detaille"):
                    fig_q2=make_kpi_bar_chart_with_target(PK,global_actuals,CIBLE,"Qualite","#3182ce","#e53e3e")
            ...
            with tab_backlog:
                ...
                if oms_pivot.empty: st.markdown('<div class="es">Aucun OT contenant "OMS" trouve.</div>',unsafe_allow_html=True)
                else:
                    st.markdown('<div style="font-size:12px;color:#718096;margin-bottom:4px">%d OT trouves avec "OMS"</div>'%int(oms_pivot.sum().sum()),unsafe_allow_html=True)
                ...
                if thermo_pivot.empty: st.markdown('<div class="es">Aucun OT contenant "THERMOGRAPHIE" trouve.</div>',unsafe_allow_html=True)
                else:
                    st.markdown('<div style="font-size:12px;color:#718096;margin-bottom:4px">%d OT trouves avec "THERMOGRAPHIE"</div>'%int(thermo_pivot.sum().sum()),unsafe_allow_html=True)
                    c1,c2=st.columns([3,2])
                ...
                if bp_pivot.empty: st.markdown('<div class="es">Aucun OT en attente de preparation.</div>',unsafe_allow_html=True)
                    ...
                    st.markdown('<div style="font-size:12px;color:#718096;margin-bottom:4px">%d OT en attente de preparation</div>'%int(bp_pivot["Total"].sum()),unsafe_allow_html=True)
                    c1,c2=st.columns([3,2])
                    ...
                    with c2:
                        ...
                        f=anl_pie_chart(d,"Statut","Nombre","Backlog Prep — % Caracterise",colors=["#38a169","#e53e3e"])
                        if f: st.plotly_chart(f,use_container_width=True)
                st.markdown('<div class="sec-sep"></div>',unsafe_allow_html=True)
                ...
                if be_pivot.empty: st.markdown('<div class="es">Aucun OT en execution.</div>',unsafe_allow_html=True)
                    ...
                    st.markdown('<div style="font-size:12px;color:#718096;margin-bottom:4px">%d OT en cours d\'execution</div>'%int(be_pivot["Total"].sum()),unsafe_allow_html=True)
                    c1,c2=st.columns([3,2])
                    ...
                    with c2:
                        ...
                        f=anl_pie_chart(d,"Age","Nombre","Backlog Exec — Repartition",colors=["#38a169","#d69e2e","#e53e3e"])
                        if f: st.plotly_chart(f,use_container_width=True)

            with tab_journal:
                st.markdown('<div class="stl s">Journal des Variations Significatives</div>',unsafe_allow_html=True)
                hist_path=os.path.join("kpis","indicateurs_kpis.xlsx"); hist_df=load_historical_kpis(hist_path)
                if hist_df.empty: st.markdown('<div class="es">Pas assez d\'historique (minimum 2 periodes)</div>',unsafe_allow_html=True)
                else:
                    ...
                    if var_df.empty: st.markdown('<div class="es">Aucune variation calculee</div>',unsafe_allow_html=True)
                        ...
                        if jrn.empty: st.markdown('<div class="es">Aucune variation significative (ecart >= 5%%)</div>',unsafe_allow_html=True)
                        else:
            ...
            with tab_classement:
                ...
                if not hist_df.empty:
                    ...
                    if not var_df.empty:
                        ...
                        c1,c2=st.columns(2)
                        with c1:
                            if not top_h.empty: st.markdown("**🟢 Top 5 Progression**"); st.dataframe(top_h,use_container_width=True)
                        with c2:
                            if not bot_h.empty: st.markdown("**🔴 Bottom 5 Regression**"); st.dataframe(bot_h,use_container_width=True)
