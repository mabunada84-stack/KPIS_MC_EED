    ...
    if not unf or (ot_f is not None and av_f is not None):
        try:
            ...
            with tabs[0]:
                ...
                else: st.markdown('<div class="es">Colonne Statut OT non disponible</div>',unsafe_allow_html=True)
                ...
                else: st.markdown('<div class="es">Aucun avis sans OT</div>',unsafe_allow_html=True)
                ...
                with col_e2: st.info("💡 Les lignes rouges pointillees = objectif cible de chaque indicateur.")
            ...
            with tabs[3]:
                ...
                if ano_p: ano_p.append({"Poste de travail":"TOTAL","Nombre anomalies":tp,"_t":"total"})
                if ano_q: ano_q.append({"Poste de travail":"TOTAL","Nombre anomalies":tq,"_t":"total"})
            ...
            with tabs[5]:
                ...
                if os.path.exists(kpis_path):
                    ...
                    else: st.markdown('<div class="es">Pas assez de donnees historiques (minimum 2 periodes)</div>',unsafe_allow_html=True)
                else: st.markdown('<div class="es">Aucun fichier historique. Sauvegardez d\'abord les KPIs.</div>',unsafe_allow_html=True)
                st.markdown("---")
            ...
            if unf: st.info("Chargez les fichiers OT et AVIS depuis la barre laterale.")
            else: st.info("Verifiez que ot.xlsx et avis.xlsx sont dans le meme repertoire.")
    else:
        if unf:
