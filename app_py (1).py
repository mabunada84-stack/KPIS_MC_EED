...
def main():
    ...
    if not st.session_state.hse_affiche:
        ...
        st.markdown("""
        <h1 style="text-align:center; font-size:60px; color:#0f172a;">
            🦺 HSE - CONSIGNE DE SÉCURITÉ
        </h1>
        """, unsafe_allow_html=True)
        st.markdown("""
        <h2 style="text-align:center; color:#64748b;">
            Sécurité • Santé • Environnement
        </h2>
        """, unsafe_allow_html=True)
        st.markdown(f"""
        <div style="background-color:#fff3cd; border-left:10px solid #ffc107; padding:35px; border-radius:15px; font-size:38px; font-weight:bold; text-align:center; margin-top:20px; margin-bottom:30px;">
            ⚠️ {consigne}
        </div>
        """, unsafe_allow_html=True)
        st.markdown("""
        <h1 style="text-align:center; color:#198754; font-size:50px; font-weight:800;">
            Aucun travail n'est plus urgent que la sécurité
        </h1>
        """, unsafe_allow_html=True)
