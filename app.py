# -*- coding: utf-8 -*-
# VERSION V2.1 - Detection automatique noms de colonnes
...
st.set_page_config(layout="wide", page_title="Dashboard KPI V2.1")
...
# ============================================================
# FONCTION CLE : Detection automatique des noms de colonnes
# ============================================================
def find_col(df, candidates):
    """Cherche la meilleure correspondance de colonne dans un DataFrame."""
    # 1. Correspondance exacte
    for c in candidates:
        if c in df.columns:
            return c
    # 2. Correspondance partielle (ignore casse)
    for c in df.columns:
        cl = str(c).lower().strip()
        for cand in candidates:
            if cand.lower().strip() in cl or cl in cand.lower().strip():
                return c
    return None
...
def main():
    ...
    def excr(df):
        col_p = find_col(df, ["Poste travail princ.", "Poste de travail", "Poste travail", "Poste"])
        if col_p:
            return df[~df[col_p].astype(str).str.contains("cresseur",case=False,na=False)].copy()
        return df
    ...
    def calc_kpis(df_i,av_i,now,posts):
        ...
        # ==== MAPPING AUTOMATIQUE DES COLONNES OT ====
        col_poste = find_col(df, ["Poste travail princ.", "Poste de travail", "Poste travail", "Poste"])
        col_statut_ot = find_col(df, ["Statut OT", "Statut système", "Statut", "Statut utilisateur"])
        col_statut_sys = find_col(df, ["Statut système", "Statut OT", "Statut"])
        col_statut_usr = find_col(df, ["Statut utilisateur", "Statut"])
        col_appel = find_col(df, ["Nº appel pl.entret.", "N° appel pl.entret.", "Appel", "Nº appel", "Numero appel"])
        col_cout_bud = find_col(df, ["Total coûts budgétés", "Total couts budgétés", "Cout budgété", "Coût budgété"])
        col_cout_reel = find_col(df, ["Total coûts réels", "Total couts reels", "Cout réel", "Coût réel"])
        col_date_create = find_col(df, ["Créé le", "Cree le", "Date de création", "Date creation"])
        col_date_debut = find_col(df, ["Date de début planifiée", "Date de début planifiee", "Date début planifiée", "Date debut planifiee"])
        col_ordre = find_col(df, ["Ordre", "N° OT", "Numero OT", "Nº OT"])

        # Renommer pour standardiser
        if col_poste: df["Poste travail princ."] = df[col_poste]
        else: df["Poste travail princ."] = "INCONNU"
        if col_statut_ot: df["Statut OT"] = df[col_statut_ot].astype(str).str.strip()
        else: df["Statut OT"] = "CRÉÉ"
        if col_statut_sys: df["Statut système"] = df[col_statut_sys].astype(str).str.strip()
        else: df["Statut système"] = ""
        if col_statut_usr: df["Statut utilisateur"] = df[col_statut_usr].astype(str).str.strip()
        else: df["Statut utilisateur"] = ""
        if col_appel: df["Nº appel pl.entret."] = df[col_appel]
        else: df["Nº appel pl.entret."] = 0
        if col_cout_bud: df["Total coûts budgétés"] = df[col_cout_bud]
        else: df["Total coûts budgétés"] = 0
        if col_cout_reel: df["Total coûts réels"] = df[col_cout_reel]
        else: df["Total coûts réels"] = 0
        if col_ordre: df["Ordre"] = df[col_ordre]
        else: df["Ordre"] = range(1, len(df)+1)

        # ==== MAPPING AUTOMATIQUE DES COLONNES AVIS ====
        av_col_poste = find_col(av, ["Poste travail princ.", "Poste de travail", "Poste travail", "Poste"])
        av_col_statut = find_col(av, ["Statut utilisateur", "Statut", "Statut système"])
        av_col_avis = find_col(av, ["Avis", "N° Avis", "Numero Avis", "Nº Avis"])
        av_col_ordre = find_col(av, ["Ordre", "N° OT", "Numero OT", "Nº OT"])

        if av_col_poste: av["Poste travail princ."] = av[av_col_poste]
        else: av["Poste travail princ."] = "INCONNU"
        if av_col_statut: av["Statut utilisateur"] = av[av_col_statut].astype(str).str.strip()
        else: av["Statut utilisateur"] = ""
        if av_col_avis: av["Avis"] = av[av_col_avis]
        else: av["Avis"] = range(1, len(av)+1)
        if av_col_ordre: av["Ordre"] = av[av_col_ordre]
        else: av["Ordre"] = None
        ...
        for dc,am,ac in [('Créé le',"amp","ap"),('Date de début planifiée',"amlp","alp"),('Date de début planifiée',"amex","aex")]:
            real_dc = col_date_create if dc=='Créé le' else col_date_debut
            if real_dc and real_dc in df.columns:
                df[real_dc]=pd.to_datetime(df[real_dc],errors='coerce')
                df[am]=((now.year-df[real_dc].dt.year)*12+(now.month-df[real_dc].dt.month)).round(2)
    ...
    def ks(v,c):
        ...
        if c in ["OT préparation 1mois< <3mois","OT planification 1mois< <3mois","OT exécution 1mois< <3mois"]:
            if val<=15: return "background:#c6efce;color:#006100;font-weight:600"
            else: return "background:#ffc7ce;color:#9c0006;font-weight:600"
        if c in ["OT préparation >3 mois","OT planification >3 mois","OT exécution >3 mois"]:
            if val<=5: return "background:#c6efce;color:#006100;font-weight:600"
            else: return "background:#ffc7ce;color:#9c0006;font-weight:600"
    ...
    def kas(v):
        ...
        elif val<=3: return "background:#ffeb9c;color:#9c6500;font-weight:600"
        elif val<=10: return "background:#fed7d7;color:#c53030;font-weight:600"
        else: return "background:#fc8181;color:#742a2a;font-weight:800"
    def gscore(k,a,t):
    ...
    def anl_pie_chart(data,names_col,values_col,title,colors=None,min_pct=3.0):
        ...
        if not has_small:
            ...
            fig.update_layout(margin=dict(t=60,b=50,l=20,r=20),height=460,autosize=True,title_font_size=15,legend=dict(font_size=11,orientation="h",yanchor="bottom",y=-0.15))
            return fig
        else:
            others_label="Autres (%d)"%len(small)
            others_row=pd.DataFrame([{names_col:others_label,values_col:small[values_col].sum()}])
            ...
            main_colors=[base_colors[i%len(base_colors)] if i<len(main_df)-1 else "#CBD5E0" for i in range(len(main_df))]
            sub_colors=[base_colors[(len(big)+i)%len(base_colors)] for i in range(len(sub_df))]
            fig=make_subplots(rows=1,cols=2,specs=[[{"type":"pie"},{"type":"pie"}]],subplot_titles=["<b>%s</b>"%title,"<b>Detail Autres</b>"],horizontal_spacing=0.08)
            fig.add_trace(go.Pie(labels=main_df[names_col].tolist(),values=main_df[values_col].tolist(),textinfo='percent+label',textposition='inside',textfont_size=12,marker_colors=main_colors,pull=[0.03 if i==len(main_df)-1 else 0.01 for i in range(len(main_df))]),row=1,col=1)
            fig.add_trace(go.Pie(labels=sub_df[names_col].tolist(),values=sub_df[values_col].tolist(),textinfo='percent+label',textposition='inside',textfont_size=11,marker_colors=sub_colors,hole=0.3),row=1,col=2)
            fig.update_layout(margin=dict(t=60,b=50,l=10,r=10),height=460,autosize=True,showlegend=True,legend=dict(font_size=10,orientation="h",yanchor="bottom",y=-0.08))
            return fig
    ...
    def make_rank_bar_chart(top_df,bottom_df,kpi_name,target_val,color_top,color_bottom):
        ...
        fig=go.Figure()
        fig.add_trace(go.Bar(y=rdf["Poste"],x=rdf["Score"],orientation='h',marker_color=bar_colors,text=["%.1f%%"%s for s in rdf["Score"]],textposition='outside',textfont=dict(size=12,color="#1a202c")))
        fig.add_vline(x=target_val,line_dash="dash",line_width=2.5,line_color="#e53e3e",annotation_text="Objectif: %d%%"%target_val,annotation_position="top right",annotation_font=dict(size=12,color="#e53e3e",weight="bold"),annotation_bgcolor="rgba(255,255,255,0.85)")
        fig.update_layout(title="<b>%s</b>"%kpi_name,height=max(220,len(rdf)*48+100),margin=dict(l=180,r=80,t=60,b=30),xaxis=dict(range=[0,max(110,rdf["Score"].max()*1.15)],gridcolor="#edf2f7",zeroline=False),yaxis=dict(tickfont=dict(size=12)),plot_bgcolor="white",showlegend=False,bargap=0.35)
        return fig
    ...
    def make_kpi_bar_chart_with_target(kpi_list,actuals,targets,title,color_ok,color_fail):
        ...
        fig=go.Figure()
        fig.add_trace(go.Bar(x=names,y=vals,marker_color=colors,text=["%.1f%%"%v for v in vals],textposition='outside',textfont=dict(size=11,color="#1a202c")))
        for i,tv in enumerate(cibles):
            fig.add_shape(type="line",x0=i-0.4,x1=i+0.4,y0=tv,y1=tv,line=dict(color="#e53e3e",width=2.5,dash="dash"))
            fig.add_annotation(x=i,y=tv,text="Cible %d%%"%tv,showarrow=False,yshift=8,font=dict(size=9,color="#e53e3e",weight="bold"),bgcolor="rgba(255,255,255,0.8)")
        fig.update_layout(title="<b>%s</b>"%title,height=420,margin=dict(l=20,r=20,t=60,b=140),xaxis=dict(tickangle=-45,tickfont=dict(size=10)),yaxis=dict(range=[0,max(max(vals),max(cibles))*1.2],title="%",gridcolor="#edf2f7"),plot_bgcolor="white",showlegend=False,bargap=0.3)
        return fig
            ...
            if unf:
                ...
                if os.path.exists("ot.xlsx"):
                    try:
                        ...
                        col_p_tmp = find_col(_t, ["Poste travail princ.", "Poste de travail", "Poste travail", "Poste"])
                        if col_p_tmp:
                            apm=sorted(_t[_t_tmp].astype(str).str.startswith(("SF1","SF2"),na=False)[col_p_tmp].dropna().unique().tolist())
                    except Exception: pass
                st.markdown("""<div style="background:rgba(255,255,255,.1);padding:6px 10px;border-radius:6px;border:1px solid rgba(255,255,255,.15)"><div style="font-size:14px;color:white;font-weight:600">📅 %s</div></div>"""%fichier_date,unsafe_allow_html=True)
    ...
    if not unf or (ot_f is not None and av_f is not None):
        try:
            ...
            # Afficher les colonnes détectées (pour debug, à enlever ensuite)
            with st.sidebar:
                with st.expander("🔍 Colonnes OT détectées"):
                    st.write(list(raw_ot.columns))
                with st.expander("🔍 Colonnes AVIS détectées"):
                    st.write(list(raw_av.columns))

            if not apm:
                col_p_tmp = find_col(raw_ot, ["Poste travail princ.", "Poste de travail", "Poste travail", "Poste"])
                if col_p_tmp:
                    apm=sorted(raw_ot[raw_ot[col_p_tmp].astype(str).str.startswith(("SF1","SF2"),na=False)][col_p_tmp].dropna().unique().tolist())
            if "All" in sp or not sp: sp_sel=apm
            else: sp_sel=[p for p in sp if p in apm]
            if "All" in sa or not sa: sa_sel=["All"]
            else: sa_sel=sa
            if "All" in sd or not sd: sd_sel=["All"]
            else: sd_sel=sd
            ...
            if not filtered_posts: filtered_posts=[p for p in apm if mf(p)]
            if not filtered_posts: filtered_posts=apm

            col_date_filt = find_col(raw_ot, ["Date de début planifiée", "Date de début planifiee", "Date début planifiée"])
            if col_date_filt:
                raw_ot[col_date_filt]=pd.to_datetime(raw_ot[col_date_filt],errors="coerce")
                df_filt=raw_ot[(raw_ot["Poste travail princ."].isin(filtered_posts))&(raw_ot[col_date_filt]>=sdt)&(raw_ot[col_date_filt]<=edt)].copy()
            else:
                df_filt=raw_ot[raw_ot["Poste travail princ."].isin(filtered_posts)].copy()

            av_filt=raw_av[raw_av["Poste travail princ."].isin(filtered_posts)].copy() if "Poste travail princ." in raw_av.columns else raw_av.copy()
            ...
            for p in filtered_posts:
                ps_sum=0; qs_sum=0; ps_n=0; qs_n=0
                for k in QK:
                    if k in ckdf.columns and p in ckdf.index:
                        try: v=float(ckdf.loc[p,k]); ps_sum+=gscore(k,v,CIBLE.get(k,100)); ps_n+=1
                        except Exception: pass
                for k in PK:
                    if k in ckdf.columns and p in ckdf.index:
                        try: v=float(ckdf.loc[p,k]); qs_sum+=gscore(k,v,CIBLE.get(k,100)); qs_n+=1
                        except Exception: pass
            ...
            total_ot=len(df_filt); total_avis=len(avf)

            st.markdown("""<div class="mh"><h1>📊 Dashboard KPI Maintenance [V2.1]</h1><span class="db">📅 %s</span></div>"""%fichier_date,unsafe_allow_html=True)
            ...
            with tabs[0]:
                ...
                if fig_qr: st.plotly_chart(fig_qr,use_container_width=True)
                ...
                if "Statut OT" in dfp.columns and not dfp.empty:
                    ...
                    fig_op=anl_pie_chart(ot_stat_df,"Statut OT","Nombre","Nombre d'OT par Statut",colors=["#38a169","#3182ce","#805ad5","#e53e3e","#d69e2e","#ed8936"])
                    if fig_op: st.plotly_chart(fig_op,use_container_width=True)
                ...
                if "Statut utilisateur" in avf.columns and not avf.empty:
                    ...
                    fig_ap=anl_pie_chart(av_stat_df,"Statut utilisateur","Nombre","Nombre d'Avis par Statut",colors=["#38a169","#3182ce","#805ad5","#e53e3e","#d69e2e","#ed8936"])
                ...
                fig_pb=make_kpi_bar_chart_with_target(QK,perf_avg,CIBLE,"Indicateurs Performance (ligne rouge = objectif)","#38a169","#e53e3e")
                ...
                fig_qb=make_kpi_bar_chart_with_target(PK,qual_avg,CIBLE,"Indicateurs Qualite (ligne rouge = objectif)","#3182ce","#e53e3e")
                if fig_qb: st.plotly_chart(fig_qb,use_container_width=True)
                ...
                with col_e1: export_btn(ckdf.round(2).reset_index(),"synthese_kpis.xlsx")
                with col_e2: st.info("💡 Lignes rouges = objectif cible.")

            with tabs[1]:
                st.markdown('<div class="stl p">📈 Indicateurs de Performance</div>',unsafe_allow_html=True)
                pcols_t=["Poste de travail"]+QK+["Score Performance"]; prows=[]
                for p in filtered_posts:
                    rd={"Poste de travail":p,"_t":""}
                    for k in QK:
                        if k in ckdf.columns and p in ckdf.index:
                            try: rd[k]=round(float(ckdf.loc[p,k]),1)
                            except Exception: rd[k]="-"
                        else: rd[k]="-"
                    rd["Score Performance"]=round(perf_scores.get(p,0),2); prows.append(rd)
                cr={"Poste de travail":"CIBLE","_t":"cible"}
                for k in QK: cr[k]=CIBLE.get(k,"-")
                cr["Score Performance"]="80%"; prows.append(cr)
                tr={"Poste de travail":"Moyenne","_t":"total"}
                for k in QK:
                    if k in ckdf.columns: tr[k]=round(ckdf[k].mean(),1)
                    else: tr[k]="-"
                tr["Score Performance"]=round(global_perf,2); prows.append(tr)
            ...
            with tabs[2]:
                ...
                qcols_t=["Poste de travail"]+PK+["Score Qualite"]; qrows=[]
                for p in filtered_posts:
                    rd={"Poste de travail":p,"_t":""}
                    for k in PK:
                        if k in ckdf.columns and p in ckdf.index:
                    ...
                    rd["Score Qualite"]=round(qual_scores.get(p,0),2); qrows.append(rd)
                cr2={"Poste de travail":"CIBLE","_t":"cible"}
                for k in PK: cr2[k]=CIBLE.get(k,"-")
                cr2["Score Qualite"]="80%"; qrows.append(cr2)
                tr2={"Poste de travail":"Moyenne","_t":"total"}
                for k in PK:
                    if k in ckdf.columns: tr2[k]=round(ckdf[k].mean(),1)
                    else: tr2[k]="-"
                tr2["Score Qualite"]=round(global_qual,2); qrows.append(tr2)
            ...
            with tabs[3]:
                    ...
                    for k in QK:
                        if k in ckdf.columns and p in ckdf.index:
                            try:
                                ...
                                if (is_lb(k) and v>CIBLE.get(k,100)) or (not is_lb(k) and v<CIBLE.get(k,0)): np_c+=1
                            except Exception: pass
                    for k in PK:
                        if k in ckdf.columns and p in ckdf.index:
                            try:
                                v=float(ckdf.loc[p,k])
                                if v<CIBLE.get(k,100): nq_c+=1
                            except Exception: pass
                    if np_c>0: ano_p.append({"Poste de travail":p,"Nombre anomalies":np_c,"_t":""})
                    if nq_c>0: ano_q.append({"Poste de travail":p,"Nombre anomalies":nq_c,"_t":""})
                ano_p.sort(key=lambda x:x["Nombre anomalies"],reverse=True)
                ano_q.sort(key=lambda x:x["Nombre anomalies"],reverse=True)
                if ano_p: ano_p.append({"Poste de travail":"TOTAL","Nombre anomalies":sum(r["Nombre anomalies"] for r in ano_p),"_t":"total"})
                if ano_q: ano_q.append({"Poste de travail":"TOTAL","Nombre anomalies":sum(r["Nombre anomalies"] for r in ano_q),"_t":"total"})
                st.markdown('<div class="stl p" style="font-size:13px">Anomalies Performance</div>',unsafe_allow_html=True)
                if ano_p: st.markdown(html_ano(ano_p,["Poste de travail","Nombre anomalies"]),unsafe_allow_html=True)
                ...
                if ano_q: st.markdown(html_ano(ano_q,["Poste de travail","Nombre anomalies"]),unsafe_allow_html=True)
                else: st.markdown('<div class="es">Aucune anomalie</div>',unsafe_allow_html=True)

            with tabs[4]:
                st.markdown('<div class="stl c">🏆 Classement General</div>',unsafe_allow_html=True)
                ...
                st.markdown('<div style="height:8px"></div>',unsafe_allow_html=True)
                st.markdown(html_grouped_bars(filtered_posts,perf_scores,qual_scores,"Comparaison Performance / Qualite"),unsafe_allow_html=True)

            with tabs[5]:
                st.markdown('<div class="stl s">📉 Variations</div>',unsafe_allow_html=True)
                kpis_path=os.path.join("kpis","indicateurs_kpis.xlsx")
                if os.path.exists(kpis_path):
                    hist_df=load_historical_kpis(kpis_path); var_df=calculate_variations(hist_df)
                    if not var_df.empty:
                        journal=generate_journal(var_df)
                        if not journal.empty: st.dataframe(journal.drop(columns=["Significatif"],errors="ignore"),use_container_width=True,height=400)
                        else: st.markdown('<div class="es">Aucune variation significative</div>',unsafe_allow_html=True)
                    else: st.markdown('<div class="es">Pas assez d\'historique</div>',unsafe_allow_html=True)
                else: st.markdown('<div class="es">Aucun fichier historique</div>',unsafe_allow_html=True)
                if st.button("💾 Sauvegarder les KPIs",key="save_hist"):
                    pcols_s=["Poste de travail"]+QK+["Score Performance"]; prows_s=[]
                    for p in filtered_posts:
                        rd={"Poste de travail":p}
                        for k in QK:
                            if k in ckdf.columns and p in ckdf.index:
                                try: rd[k]=round(float(ckdf.loc[p,k]),1)
                                except Exception: rd[k]=0
                            else: rd[k]=0
                        rd["Score Performance"]=round(perf_scores.get(p,0),2); prows_s.append(rd)
                    qcols_s=["Poste de travail"]+PK+["Score Qualite"]; qrows_s=[]
                    for p in filtered_posts:
                        rd={"Poste de travail":p}
                        for k in PK:
                            if k in ckdf.columns and p in ckdf.index:
                        ...
                        rd["Score Qualite"]=round(qual_scores.get(p,0),2); qrows_s.append(rd)
                    save_kpis_to_excel(prows_s,pcols_s,qrows_s,qcols_s,ano_p,["Poste de travail","Nombre anomalies"] if ano_p else [],ano_q,["Poste de travail","Nombre anomalies"] if ano_q else [],fichier_date)
                    st.success("✅ KPIs sauvegardes")
        ...
        if unf:
            ...
            <p style="font-size:16px;color:#718096;margin-top:10px">Veuillez charger les fichiers OT et AVIS.</p></div>""",unsafe_allow_html=True)
