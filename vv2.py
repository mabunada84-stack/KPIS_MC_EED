# -*- coding: utf-8 -*-
# VERSION V3.0 - Suppression Vue Globale, Synthese & Actions premier onglet, Top5/Bottom5 Perf & Qual
...
def save_kpis_to_excel(prows,pcols,qrows,qcols,ano_p_r,ano_p_c,ano_q_r,ano_q_c,sheet_name):
    ...
    def ws_sec(title,cols,rows,sr):
        ...
        for j,c in enumerate(cols,1):
            ...
            cl.font=hf; cl.fill=hfl; cl.alignment=Alignment(horizontal='center'); cl.border=tb
        sr+=1
        for r in rows:
            for j,c in enumerate(cols,1):
...
def main():
    ...
    if not unf or (ot_f is not None and av_f is not None):
        try:
            ...
            if sa and "All" not in sa:
                ...
                if "Poste travail princ." in df_ot.columns:
                    df_ot=df_ot[df_ot["Poste travail princ."].str.contains(afilter,case=False,na=False)]
                if "Poste travail princ." in df_av.columns:
                    df_av=df_av[df_av["Poste travail princ."].str.contains(afilter,case=False,na=False)]
            if sd and "All" not in sd:
                dfilter="|".join(sd)
                if "Poste travail princ." in df_ot.columns:
                    df_ot=df_ot[df_ot["Poste travail princ."].str.contains(dfilter,case=False,na=False)]
                if "Poste travail princ." in df_av.columns:
                    df_av=df_av[df_av["Poste travail princ."].str.contains(dfilter,case=False,na=False)]
            ...
            # ---- Calcul scores par poste ----
            perf_scores={}; qual_scores={}
            for p in posts:
                if p in ckdf.index:
                    ps=0; qs=0
                    for k in QK:
                        a=ckdf.loc[p,k] if k in ckdf.columns else 0
                        t=CIBLE.get(k,100)
                        ps+=gscore(k,a,t)
                    for k in PK:
                        a=ckdf.loc[p,k] if k in ckdf.columns else 0
                        t=CIBLE.get(k,100)
                        qs+=gscore(k,a,t)
                    perf_scores[p]=ps/len(QK)*100 if QK else 0
                    qual_scores[p]=qs/len(PK)*100 if PK else 0
                else:
            ...
            # ---- Moyennes globales par KPI ----
            global_actuals={}
            for k in ALL_KPI:
                if k in ckdf.columns:
                    global_actuals[k]=round(ckdf[k].mean(),1)
                else:
                    global_actuals[k]=0
            ...
            # ---- Donnees pour Pie OT par statut ----
            ...
            # ---- Donnees pour Pie Avis par statut ----
            ...
            if ano_p_rows:
                ano_p_rows.append({"Poste de travail":"Total","KPI":"","Valeur":"","Cible":"","Ecart":tot_ano_p})
                ano_p_rows[-1]["_t"]="total"
            if ano_q_rows:
                ano_q_rows.append({"Poste de travail":"Total","KPI":"","Valeur":"","Cible":"","Ecart":tot_ano_q})
                ano_q_rows[-1]["_t"]="total"

            # ---- Construction tables Performance & Qualite ----
            ...
            for p in posts:
                if p in ckdf.index:
                    ...
                    r["Score Performance"]=round(perf_scores[p],1)
                    prows.append(r)
            ...
            for p in posts:
                if p in ckdf.index:
                    ...
                    r["Score Qualite"]=round(qual_scores[p],1)
                    qrows.append(r)
            ...
            with tab_synthese:
                st.markdown('<div class="stl s">Synthese & Actions — Vue globale avec pie charts, objectifs et classements</div>',unsafe_allow_html=True)
                ...
                # --- Ligne 2 : Barres KPI avec objectifs ---
                st.markdown('<div class="stl p">Indicateurs de Performance — avec Objectifs</div>',unsafe_allow_html=True)
                fig_perf_bar=make_kpi_bar_chart_with_target(QK,global_actuals,CIBLE,
                    "Performance — Valeurs vs Cibles","#38a169","#e53e3e")
                if fig_perf_bar: st.plotly_chart(fig_perf_bar,use_container_width=True)

                st.markdown('<div class="stl q">Indicateurs de Qualite — avec Objectifs</div>',unsafe_allow_html=True)
                fig_qual_bar=make_kpi_bar_chart_with_target(PK,global_actuals,CIBLE,
                    "Qualite — Valeurs vs Cibles","#3182ce","#e53e3e")
                if fig_qual_bar: st.plotly_chart(fig_qual_bar,use_container_width=True)

                # --- Ligne 3 : Top 5 / Bottom 5 Performance ---
                st.markdown('<div class="stl p">Classement Performance — Top 5 / Bottom 5</div>',unsafe_allow_html=True)
                fig_tp=make_rank_bar_chart(top5_perf,bot5_perf,
                    "Top 5 / Bottom 5 — Score Performance","#38a169","#e53e3e")
                ...
                with col_a1:
                    ...
                    if non_atteints_p:
                        st.markdown(html_actions_table(non_atteints_p,global_actuals,CIBLE,ACT_MAP),unsafe_allow_html=True)
                    else:
                        st.markdown('<div class="es">✅ Tous les KPI Performance atteints</div>',unsafe_allow_html=True)
                    st.markdown('</div>',unsafe_allow_html=True)
                with col_a2:
                    st.markdown('<div class="ca"><div class="ct" style="color:#3182ce">Actions Qualite</div>',unsafe_allow_html=True)
                    if non_atteints_q:
                        st.markdown(html_actions_table(non_atteints_q,global_actuals,CIBLE,ACT_MAP),unsafe_allow_html=True)
                    else:
                        st.markdown('<div class="es">✅ Tous les KPI Qualite atteints</div>',unsafe_allow_html=True)
                    st.markdown('</div>',unsafe_allow_html=True)

                # --- Ligne 6 : Barres groupees Performance vs Qualite ---
            ...
            with tab_performance:
                ...
                with st.expander("📊 Graphique detaille Performance"):
                    fig_p2=make_kpi_bar_chart_with_target(QK,global_actuals,CIBLE,
                        "Performance — Detail KPI","#38a169","#e53e3e")
            ...
            with tab_qualite:
                ...
                with st.expander("📊 Graphique detaille Qualite"):
                    fig_q2=make_kpi_bar_chart_with_target(PK,global_actuals,CIBLE,
                        "Qualite — Detail KPI","#3182ce","#e53e3e")
            ...
            with tab_anomalies:
                ...
                with col_an1:
                    st.markdown('<div style="font-size:13px;font-weight:700;color:#38a169;margin-bottom:4px">Anomalies Performance (%d)</div>'%tot_ano_p,unsafe_allow_html=True)
                    if ano_p_rows:
                ...
                with col_an2:
                    st.markdown('<div style="font-size:13px;font-weight:700;color:#3182ce;margin-bottom:4px">Anomalies Qualite (%d)</div>'%tot_ano_q,unsafe_allow_html=True)
                    if ano_q_rows:
        ...
        if unf:
            ...
            <h2 style="color:#1e3a5f">Chargement de fichiers</h2>
            <p>Veuillez charger les fichiers OT et AVIS dans la barre laterale.</p></div>""",unsafe_allow_html=True)
        else:
            st.markdown("""<div style="text-align:center;padding:60px;color:#718096">
            <div style="font-size:64px;margin-bottom:16px">📂</div>
            <h2 style="color:#1e3a5f">Fichiers non trouves</h2>
            <p>Placez les fichiers <b>ot.xlsx</b> et <b>avis.xlsx</b> a la racine du projet.</p></div>""",unsafe_allow_html=True)
