...
def inject_custom_css():
    ...
    <style>
        ...
        .section-title.qty { border-left-color: #3182ce; }
        .section-title.perf { border-left-color: #38a169; }
        .section-title.ano { border-left-color: #e53e3e; }
        .chart-container {
        ...
        .synthese-row.qty-row:hover { border-left-color: #3182ce; }
        .synthese-row.perf-row:hover { border-left-color: #38a169; }
        ...
        .synthese-count.qty-count { background: #3182ce; }
        .synthese-count.perf-count { background: #38a169; }
        ...
        /* Summary cards */
        .kpi-cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 14px; margin-bottom: 24px; }
        .kpi-card {
        ...
        .kpi-card .card-icon { font-size: 22px; margin-bottom: 8px; }
        .kpi-card .card-value { font-size: 28px; font-weight: 900; color: var(--text); line-height: 1; }
        .kpi-card .card-label { font-size: 11px; color: var(--text-secondary); margin-top: 4px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
        .kpi-card .card-badge {
            position: absolute; top: 14px; right: 14px; padding: 3px 10px;
            border-radius: 20px; font-size: 10px; font-weight: 700;
        }
        .kpi-card .card-badge.green { background: #c6efce; color: #006100; }
        .kpi-card .card-badge.yellow { background: #ffeb9c; color: #9c6500; }
        .kpi-card .card-badge.red { background: #ffc7ce; color: #9c0006; }
        .kpi-card.qty-card { border-top: 3px solid #3182ce; }
        .kpi-card.perf-card { border-top: 3px solid #38a169; }
        .kpi-card.qty-card .card-value { color: #2b6cb0; }
        .kpi-card.perf-card .card-value { color: #276749; }
        ...
        .table-full-width.qty-table thead th { background: linear-gradient(135deg, #2b6cb0, #3182ce); }
        .table-full-width.perf-table thead th { background: linear-gradient(135deg, #276749, #38a169); }
        .table-full-width.ano-table thead th { background: linear-gradient(135deg, #c53030, #e53e3e); }
...
def main():
    ...
    def df_to_html_kpi(df, table_class="table-full-width"):
        df = df.rename_axis("Poste de travail").reset_index()
        h = f'<table class="{table_class}"><thead><tr>' + ''.join(f'<th>{c}</th>' for c in df.columns) + '</tr></thead><tbody>'
    ...
    def df_to_html_anomalies(df):
        df = df.rename_axis("Poste de travail").reset_index()
        h = '<table class="table-full-width ano-table"><thead><tr>' + ''.join(f'<th>{c}</th>' for c in df.columns) + '</tr></thead><tbody>'
        for i, r in df.iterrows():
    ...
    def df_to_html_class(df, highlight_col="both"):
        ...
        for i, r in df.iterrows():
            ...
            for c in df.columns:
                ...
                if is_t:
                    ...
                    if highlight_col == "qty" and c == "Score KPIs Quantité":
                        s = get_class_style(v)
                    elif highlight_col == "perf" and c == "Score KPIs Qualité":
                        s = get_class_style(v)
                    elif highlight_col == "both" and c in ["Score KPIs Quantité", "Score KPIs Qualité"]:
                        s = get_class_style(v)
                    else:
                        s = "color:#a0aec0;" if c in ["Score KPIs Quantité", "Score KPIs Qualité"] and highlight_col != "both" else ""
    ...
    def render_kpi_cards(metrics, card_class, icon_map):
        """Render summary KPI cards as HTML"""
        html = '<div class="kpi-cards">'
        for kpi_name, actual_val, target_val in metrics:
            try:
                av = float(actual_val)
                tv = float(target_val)
            except:
                av = 0; tv = 0
            if av >= tv:
                badge = '<span class="card-badge green">✓ ATTEINT</span>'
            elif av >= tv * 0.95:
                badge = '<span class="card-badge yellow">⚡ PROCHE</span>'
            else:
                badge = '<span class="card-badge red">✗ SOUS CIBLE</span>'
            icon = icon_map.get(kpi_name, "📊")
            short_name = kpi_name.replace("OT ", "").replace("TAUX_REALISATION_", "Taux Réal. ").replace("CORRECTIF/PT", "Corr./PT")
            if len(short_name) > 28: short_name = short_name[:26] + "..."
            html += f'''<div class="kpi-card {card_class}">
                <div class="card-icon">{icon}</div>
                <div class="card-value">{av:.1f}%</div>
                <div class="card-label">{short_name}</div>
                {badge}
            </div>'''
        html += '</div>'
        return html
    ...
    if not use_new_files or (ot_file is not None and avis_file is not None):
        try:
            ...
            # =====================================================================
            # SÉPARATION QUANTITÉ / PERFORMANCE
            # =====================================================================
            ...
            # Split cible
            cible_qty = cible[qty_k].copy()
            cible_perf = cible[perf_k].copy()

            # Split calculated KPIs
            ...
            # Final split tables
            final_kpi_qty = pd.concat([cible_qty, calc_qty, tg_qty]).round(2)
            final_kpi_perf = pd.concat([cible_perf, calc_perf, tg_perf]).round(2)
            ...
            for poste in valid_postes:
                ...
                for kn, cc, sub, act in [
                    ...
                    if pd.notna(vk) and vk < cible.loc['CIBLE', cc]:
                        cnt = len(sub)
                        if cnt > 0: ano_ot.append({"Poste travail princ.": poste, "KPI": kn, "Nb OT impactés": cnt, "Action Suggérée": act, "Type": "Quantité" if kn in qty_k else "Performance"})
                ...
                if pd.notna(va) and va < cible.loc['CIBLE', "appel avis approuvé"]:
                    cnt = len(ap)
                    if cnt > 0: ano_av.append({"Poste travail princ.": poste, "KPI": "appel avis approuvé", "Nb OT impactés": cnt, "Action Suggérée": "Créer un OT pour les avis sans ordre.", "Type": "Performance"})
            ...
            # Split anomalies par type
            ano_qty_cols = [c for c in ano_dash.columns if c in qty_k or c == "Total éléments impactés"]
            ano_perf_cols = [c for c in ano_dash.columns if c in perf_k or c == "Nb Avis sans ordre" or c == "Total éléments impactés"]
            ano_dash_qty = ano_dash[[c for c in ano_qty_cols if c in ano_dash.columns]].copy() if not ano_dash.empty else pd.DataFrame()
            ano_dash_perf = ano_dash[[c for c in ano_perf_cols if c in ano_dash.columns]].copy() if not ano_dash.empty else pd.DataFrame()
            # Recalculate totals for split tables
            if not ano_dash_qty.empty and "Total éléments impactés" in ano_dash_qty.columns:
                ano_dash_qty["Total éléments impactés"] = ano_dash_qty.drop(columns="Total éléments impactés", errors='ignore').sum(axis=1)
            if not ano_dash_perf.empty and "Total éléments impactés" in ano_dash_perf.columns:
                ano_dash_perf["Total éléments impactés"] = ano_dash_perf.drop(columns="Total éléments impactés", errors='ignore').sum(axis=1)

            all_ano = pd.concat([df_ano_ot, df_ano_av], ignore_index=True) if not df_ano_ot.empty or not df_ano_av.empty else pd.DataFrame()
            synthese = all_ano.groupby(["KPI", "Action Suggérée", "Type"])["Nb OT impactés"].sum().reset_index().sort_values("Nb OT impactés", ascending=False).reset_index(drop=True) if not all_ano.empty else pd.DataFrame()
            synthese_qty = synthese[synthese["Type"] == "Quantité"].drop(columns="Type", errors='ignore') if not synthese.empty else pd.DataFrame()
            synthese_perf = synthese[synthese["Type"] == "Performance"].drop(columns="Type", errors='ignore') if not synthese.empty else pd.DataFrame()
            ...
            for poste in calculated_kpis_df.index:
                ...
                pl = (sl / len(perf_k) * 100) if perf_k else 0
            ...
            # Classement Quantité seul
            df_cl_qty = df_class[["Poste travail princ.", "Score KPIs Quantité"]].sort_values("Score KPIs Quantité", ascending=False).copy()
            df_cl_qty["Score KPIs Quantité"] = df_cl_qty["Score KPIs Quantité"].apply(lambda x: f"{x:.2f} %")
            tg_cl_qty = pd.DataFrame([{"Poste travail princ.": "Total général", "Score KPIs Quantité": f"{df_class['Score KPIs Quantité'].mean():.2f} %"}])
            df_cl_qty = pd.concat([df_cl_qty, tg_cl_qty], ignore_index=True)

            # Classement Performance seul
            df_cl_perf = df_class[["Poste travail princ.", "Score KPIs Qualité"]].sort_values("Score KPIs Qualité", ascending=False).copy()
            df_cl_perf["Score KPIs Qualité"] = df_cl_perf["Score KPIs Qualité"].apply(lambda x: f"{x:.2f} %")
            tg_cl_perf = pd.DataFrame([{"Poste travail princ.": "Total général", "Score KPIs Qualité": f"{df_class['Score KPIs Qualité'].mean():.2f} %"}])
            df_cl_perf = pd.concat([df_cl_perf, tg_cl_perf], ignore_index=True)

            # =====================================================================
            # CARDS DATA
            # =====================================================================
            qty_icon_map = {
                "TAUX_REALISATION_CORRECTIF/PT": "🎯", "OT préparation <1 mois": "⏱️",
                "OT préparation >3 mois": "⚠️", "OT préparation 1mois< <3mois": "🔄",
            ...
            perf_icon_map = {
                ...
                "Backlog préparation caractérisé": "🏷️", "Backlog planification caractérisé": "🏷️",
                "OT CONFIME": "公证", "OT_COR_EGAL": "⚖️"
            }

            qty_card_metrics = []
            for k in qty_k:
                av = tg_qty[k].values[0] if not tg_qty.empty else 0
                tv = cible_qty[k].values[0] if not cible_qty.empty else 100
                qty_card_metrics.append((k, av, tv))

            perf_card_metrics = []
            for k in perf_k:
                av = tg_perf[k].values[0] if not tg_perf.empty else 0
                tv = cible_perf[k].values[0] if not cible_perf.empty else 100
                perf_card_metrics.append((k, av, tv))
            ...
            # =====================================================================
            # ONGLETS : QUANTITÉ | PERFORMANCE | ANOMALIES
            # =====================================================================
            tab_qty, tab_perf, tab_ano = st.tabs([
            ...
            with tab_qty:
                # Cards résumé
                st.markdown(render_kpi_cards(qty_card_metrics, "qty-card", qty_icon_map), unsafe_allow_html=True)

                # Tableau KPI Quantité
                st.markdown('<p class="section-title qty">Indicateurs de Quantité par Poste de Travail</p>', unsafe_allow_html=True)
                st.markdown(df_to_html_kpi(final_kpi_qty, "table-full-width qty-table"), unsafe_allow_html=True)

                st.markdown("<div style='height:20px;'></div>", unsafe_allow_html=True)

                # Classement Quantité
                st.markdown('<p class="section-title qty">🏆 Classement des Postes — Score Quantité</p>', unsafe_allow_html=True)
                st.markdown(df_to_html_class(df_cl_qty, highlight_col="qty"), unsafe_allow_html=True)

                st.markdown("<div style='height:20px;'></div>", unsafe_allow_html=True)

                # Graphiques Quantité
                st.markdown('<p class="section-title qty">📊 Analyse Quantité par Catégorie</p>', unsafe_allow_html=True)
                c1, c2, c3 = st.columns(3)

                with c1:
                    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
                    st.markdown('<div class="chart-title" style="color:#2b6cb0;">🔧 Par Métier</div>', unsafe_allow_html=True)
                    df_m = df_class.groupby("Métier")["Score KPIs Quantité"].mean().reset_index().round(1)
                    df_m.columns = ["Catégorie", "Score"]
                    if not df_m.empty:
                        b = alt.Chart(df_m).mark_bar(cornerRadiusTopLeft=5, cornerRadiusTopRight=5).encode(
                            x=alt.X('Score:Q', scale=alt.Scale(domain=[0, 100], clamp=True), title=''),
                            y=alt.Y('Catégorie:O', sort='-x', title='', axis=alt.Axis(labelFontSize=12)),
                            color=alt.Color('Score:Q', scale=alt.Scale(domain=[0, 50, 80, 100], range=['#e53e3e', '#ed8936', '#ecc94b', '#3182ce']), legend=None),
                            tooltip=['Catégorie', 'Score']).properties(height=max(120, 40 * len(df_m)))
                        t = alt.Chart(df_m).mark_text(align='left', baseline='middle', dx=6, fontSize=13, fontWeight=700, color='#2b6cb0').encode(
                            y=alt.Y('Catégorie:O', sort='-x'), text=alt.Text('Score:Q', format='.1f'))
                ...
                with c2:
                    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
                    st.markdown('<div class="chart-title" style="color:#2b6cb0;">🏭 Par Atelier</div>', unsafe_allow_html=True)
                    df_a = df_class.groupby("Atelier")["Score KPIs Quantité"].mean().reset_index().round(1)
                    df_a.columns = ["Catégorie", "Score"]
                    if not df_a.empty:
                        b = alt.Chart(df_a).mark_bar(cornerRadiusTopLeft=5, cornerRadiusTopRight=5).encode(
                            x=alt.X('Score:Q', scale=alt.Scale(domain=[0, 100], clamp=True), title=''),
                            y=alt.Y('Catégorie:O', sort='-x', title='', axis=alt.Axis(labelFontSize=12)),
                            color=alt.Color('Score:Q', scale=alt.Scale(domain=[0, 50, 80, 100], range=['#e53e3e', '#ed8936', '#ecc94b', '#3182ce']), legend=None),
                            tooltip=['Catégorie', 'Score']).properties(height=max(120, 40 * len(df_a)))
                        t = alt.Chart(df_a).mark_text(align='left', baseline='middle', dx=6, fontSize=13, fontWeight=700, color='#2b6cb0').encode(
                            y=alt.Y('Catégorie:O', sort='-x'), text=alt.Text('Score:Q', format='.1f'))
                        st.altair_chart((b+t).configure_axis(grid=False, ticks=False, domain=False).configure_view(stroke='transparent'), use_container_width=True)
                    else: st.info("Aucune donnée")
                    st.markdown('</div>', unsafe_allow_html=True)
                ...
                with c3:
                    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
                    st.markdown('<div class="chart-title" style="color:#2b6cb0;">🏢 Par Division</div>', unsafe_allow_html=True)
                    df_d = df_class.groupby("Division")["Score KPIs Quantité"].mean().reset_index().round(1)
                    df_d.columns = ["Catégorie", "Score"]
                    if not df_d.empty:
                        b = alt.Chart(df_d).mark_bar(cornerRadiusTopLeft=5, cornerRadiusTopRight=5).encode(
                            x=alt.X('Score:Q', scale=alt.Scale(domain=[0, 100], clamp=True), title=''),
                            y=alt.Y('Catégorie:O', sort='-x', title='', axis=alt.Axis(labelFontSize=12)),
                            color=alt.Color('Score:Q', scale=alt.Scale(domain=[0, 50, 80, 100], range=['#e53e3e', '#ed8936', '#ecc94b', '#3182ce']), legend=None),
                            tooltip=['Catégorie', 'Score']).properties(height=max(120, 40 * len(df_d)))
                        t = alt.Chart(df_d).mark_text(align='left', baseline='middle', dx=6, fontSize=13, fontWeight=700, color='#2b6cb0').encode(
                            y=alt.Y('Catégorie:O', sort='-x'), text=alt.Text('Score:Q', format='.1f'))
                        st.altair_chart((b+t).configure_axis(grid=False, ticks=False, domain=False).configure_view(stroke='transparent'), use_container_width=True)
                    else: st.info("Aucune donnée")
                    st.markdown('</div>', unsafe_allow_html=True)

                # Synthèse actions Quantité
                st.markdown("<div style='height:20px;'></div>", unsafe_allow_html=True)
                st.markdown('<p class="section-title qty">📋 Synthèse des Actions — Quantité</p>', unsafe_allow_html=True)
                if not synthese_qty.empty:
                    for i, r in synthese_qty.iterrows():
                        ...
                        st.markdown(f"""<div class="synthese-row qty-row"><div class="synthese-kpi-name">{r['KPI']}</div><div class="synthese-count qty-count">{nb}</div><div class="synthese-action">{r['Action Suggérée']}</div><span class="top-badge {bg}">{lb}</span></div>""", unsafe_allow_html=True)
                else:
                    st.markdown("""<div class="empty-state" style="padding:30px;"><div class="icon">🎉</div><h3>Aucune action requise</h3><p>Tous les indicateurs de quantité sont dans le vert.</p></div>""", unsafe_allow_html=True)
            ...
            with tab_perf:
                # Cards résumé
                st.markdown(render_kpi_cards(perf_card_metrics, "perf-card", perf_icon_map), unsafe_allow_html=True)

                # Tableau KPI Performance
                st.markdown('<p class="section-title perf">Indicateurs de Performance par Poste de Travail</p>', unsafe_allow_html=True)
                st.markdown(df_to_html_kpi(final_kpi_perf, "table-full-width perf-table"), unsafe_allow_html=True)

                st.markdown("<div style='height:20px;'></div>", unsafe_allow_html=True)

                # Classement Performance
                st.markdown('<p class="section-title perf">🏆 Classement des Postes — Score Performance</p>', unsafe_allow_html=True)
                st.markdown(df_to_html_class(df_cl_perf, highlight_col="perf"), unsafe_allow_html=True)

                st.markdown("<div style='height:20px;'></div>", unsafe_allow_html=True)

                # Graphiques Performance
                st.markdown('<p class="section-title perf">📊 Analyse Performance par Catégorie</p>', unsafe_allow_html=True)
                c1, c2, c3 = st.columns(3)

                with c1:
                    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
                    st.markdown('<div class="chart-title" style="color:#276749;">🔧 Par Métier</div>', unsafe_allow_html=True)
                    df_m = df_class.groupby("Métier")["Score KPIs Qualité"].mean().reset_index().round(1)
                    df_m.columns = ["Catégorie", "Score"]
                    if not df_m.empty:
                        b = alt.Chart(df_m).mark_bar(cornerRadiusTopLeft=5, cornerRadiusTopRight=5).encode(
                            x=alt.X('Score:Q', scale=alt.Scale(domain=[0, 100], clamp=True), title=''),
                            y=alt.Y('Catégorie:O', sort='-x', title='', axis=alt.Axis(labelFontSize=12)),
                            color=alt.Color('Score:Q', scale=alt.Scale(domain=[0, 50, 80, 100], range=['#e53e3e', '#ed8936', '#ecc94b', '#38a169']), legend=None),
                            tooltip=['Catégorie', 'Score']).properties(height=max(120, 40 * len(df_m)))
                        t = alt.Chart(df_m).mark_text(align='left', baseline='middle', dx=6, fontSize=13, fontWeight=700, color='#276749').encode(
                            y=alt.Y('Catégorie:O', sort='-x'), text=alt.Text('Score:Q', format='.1f'))
                        st.altair_chart((b+t).configure_axis(grid=False, ticks=False, domain=False).configure_view(stroke='transparent'), use_container_width=True)
                    else: st.info("Aucune donnée")
                    st.markdown('</div>', unsafe_allow_html=True)
                ...
                with c2:
                    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
                    st.markdown('<div class="chart-title" style="color:#276749;">🏭 Par Atelier</div>', unsafe_allow_html=True)
                    df_a = df_class.groupby("Atelier")["Score KPIs Qualité"].mean().reset_index().round(1)
                    df_a.columns = ["Catégorie", "Score"]
                    if not df_a.empty:
                        b = alt.Chart(df_a).mark_bar(cornerRadiusTopLeft=5, cornerRadiusTopRight=5).encode(
                            x=alt.X('Score:Q', scale=alt.Scale(domain=[0, 100], clamp=True), title=''),
                            ...
                            color=alt.Color('Score:Q', scale=alt.Scale(domain=[0, 50, 80, 100], range=['#e53e3e', '#ed8936', '#ecc94b', '#38a169']), legend=None),
                            tooltip=['Catégorie', 'Score']).properties(height=max(120, 40 * len(df_a)))
                        t = alt.Chart(df_a).mark_text(align='left', baseline='middle', dx=6, fontSize=13, fontWeight=700, color='#276749').encode(
                            y=alt.Y('Catégorie:O', sort='-x'), text=alt.Text('Score:Q', format='.1f'))
                        st.altair_chart((b+t).configure_axis(grid=False, ticks=False, domain=False).configure_view(stroke='transparent'), use_container_width=True)
                    else: st.info("Aucune donnée")
                    st.markdown('</div>', unsafe_allow_html=True)
                ...
                with c3:
                    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
                    st.markdown('<div class="chart-title" style="color:#276749;">🏢 Par Division</div>', unsafe_allow_html=True)
                    df_d = df_class.groupby("Division")["Score KPIs Qualité"].mean().reset_index().round(1)
                    df_d.columns = ["Catégorie", "Score"]
                    if not df_d.empty:
                        b = alt.Chart(df_d).mark_bar(cornerRadiusTopLeft=5, cornerRadiusTopRight=5).encode(
                            x=alt.X('Score:Q', scale=alt.Scale(domain=[0, 100], clamp=True), title=''),
                            ...
                            color=alt.Color('Score:Q', scale=alt.Scale(domain=[0, 50, 80, 100], range=['#e53e3e', '#ed8936', '#ecc94b', '#38a169']), legend=None),
                            tooltip=['Catégorie', 'Score']).properties(height=max(120, 40 * len(df_d)))
                        t = alt.Chart(df_d).mark_text(align='left', baseline='middle', dx=6, fontSize=13, fontWeight=700, color='#276749').encode(
                            y=alt.Y('Catégorie:O', sort='-x'), text=alt.Text('Score:Q', format='.1f'))
                        st.altair_chart((b+t).configure_axis(grid=False, ticks=False, domain=False).configure_view(stroke='transparent'), use_container_width=True)
                    else: st.info("Aucune donnée")
                    st.markdown('</div>', unsafe_allow_html=True)

                # Synthèse actions Performance
                st.markdown("<div style='height:20px;'></div>", unsafe_allow_html=True)
                st.markdown('<p class="section-title perf">📋 Synthèse des Actions — Performance</p>', unsafe_allow_html=True)
                if not synthese_perf.empty:
                    for i, r in synthese_perf.iterrows():
                        nb = int(r["Nb OT impactés"])
                        bg = "red" if nb >= 20 else ("orange" if nb >= 5 else "yellow")
                        lb = "Critique" if nb >= 20 else ("Moyen" if nb >= 5 else "Mineur")
                        st.markdown(f"""<div class="synthese-row perf-row"><div class="synthese-kpi-name">{r['KPI']}</div><div class="synthese-count perf-count">{nb}</div><div class="synthese-action">{r['Action Suggérée']}</div><span class="top-badge {bg}">{lb}</span></div>""", unsafe_allow_html=True)
                else:
                    st.markdown("""<div class="empty-state" style="padding:30px;"><div class="icon">🎉</div><h3>Aucune action requise</h3><p>Tous les indicateurs de performance sont dans le vert.</p></div>""", unsafe_allow_html=True)

            # -----------------------------------------------------------------
            # ONGLET ANOMALIES (global, avec sous-sections)
            # -----------------------------------------------------------------
            with tab_ano:
                # Sous-onglets internes via colonnes visuelles
                st.markdown('<p class="section-title ano">Anomalies Quantité</p>', unsafe_allow_html=True)
                if not ano_dash_qty.empty and ano_dash_qty.drop(columns="Total éléments impactés", errors='ignore').sum().sum() > 0:
                    st.markdown(df_to_html_anomalies(ano_dash_qty), unsafe_allow_html=True)
                else:
                    st.markdown("""<div class="empty-state" style="padding:20px;"><div class="icon">✅</div><h3>Aucune anomalie de quantité</h3><p>Tous les KPIs de quantité atteignent leurs cibles.</p></div>""", unsafe_allow_html=True)

                st.markdown("<div style='height:24px;'></div>", unsafe_allow_html=True)
                st.markdown('<p class="section-title ano">Anomalies Performance</p>', unsafe_allow_html=True)
                if not ano_dash_perf.empty and ano_dash_perf.drop(columns="Total éléments impactés", errors='ignore').sum().sum() > 0:
                    st.markdown(df_to_html_anomalies(ano_dash_perf), unsafe_allow_html=True)
                else:
                    st.markdown("""<div class="empty-state" style="padding:20px;"><div class="icon">✅</div><h3>Aucune anomalie de performance</h3><p>Tous les KPIs de performance atteignent leurs cibles.</p></div>""", unsafe_allow_html=True)
                            ...
                            for pe in (postes_ano if sel_exp == "📌 Tous les postes" else [sel_exp]):
                                ...
                                for kpi in kds:
                                    ...
                                    if kpi != "appel avis approuvé":
                                        ...
                                        apf = results['avis_df_filtered'][results['avis_df_filtered']["Poste travail princ."] == pe].copy()
                                        sd = apf[["Avis", "Poste travail princ.", "Statut utilisateur", "Créé le", "Début souhaité"]].copy()
                                    if not sd.empty:
                                        sname = f"{pe[:31]}_{kpi[:20]}"[:31]
                                        sd.to_excel(w, sheet_name=sname, index=False)

                            # Feuille récap KPI Quantité
                            final_kpi_qty.rename_axis("Poste de travail").reset_index().to_excel(w, sheet_name="KPIs Quantité", index=False)
                            # Feuille récap KPI Performance
                            final_kpi_perf.rename_axis("Poste de travail").reset_index().to_excel(w, sheet_name="KPIs Performance", index=False)
                            # Feuille classement
                            df_class.to_excel(w, sheet_name="Classement", index=False)

                        out.seek(0)
                        st.download_button(
                            label="⬇️ Télécharger le fichier Excel",
                            data=out,
                            file_name=f"Plan_Action_KPIs_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                        )
                        st.success("✅ Fichier généré avec succès !")
            else:
                st.markdown("""<div class="empty-state" style="padding:30px;"><div class="icon">🎉</div><h3>Aucun plan d'action à exporter</h3><p>Tous les indicateurs sont conformes.</p></div>""", unsafe_allow_html=True)
