# -*- coding: utf-8 -*-
# VERSION V3.2 - Nouvelle page Appel Analyse Backlog (OMS, Thermographie, Backlog Prep, Backlog Exec)
...
st.set_page_config(layout="wide", page_title="Dashboard KPI V3.2")
...
def calculate_variations(hist_df):
    ...
    for i in range(1,len(dates)):
        ...
        for sec_name,pd_d,cd_d,kl in [("Performance",pp,cp,QK+["Score Performance"]),("Qualite",pq,cq,PK+["Score Qualite"])]:
...
def calculate_rankings(var_df):
    ...
    for poste in var_df["Poste"].unique():
        ...
        for _,r in pv.iterrows():
            s=s+(-r["Ecart %"]) if r["KPI"] in LOWER_BETTER else s+r["Ecart %"]
        scores[poste]=s
...
def inject_custom_css():
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .stl.th{border-left-color:#2b6cb0}.stl.bp{border-left-color:#6b46c1}.stl.be{border-left-color:#c05621}
    ...
    .tw tbody tr:nth-child(even):hover td{background:#ebf8ff!important}
...
def main():
    ...
    def html_ano_transposed(ano_rows,kpi_list,css_class,cell_class):
        ...
        legend=('<div class="ano-legend" style="margin-bottom:6px"><span><i style="background:#fed7d7;border:1px solid #feb2b2"></i> Ecart &gt; 10 pts</span>'
                '<span><i style="background:#ffeb9c;border:1px solid #f6e05e"></i> Ecart 5-10 pts</span>'
                '<span><i style="background:#feebc8;border:1px solid #fbd38d"></i> Ecart &lt; 5 pts</span></div>')
        h=legend+'<table class="tw %s"><thead><tr><th style="min-width:200px;text-align:left">KPI</th>'%css_class
        ...
        for kpi in all_kpis:
            ...
            h+='<td style="font-weight:800;text-align:center;%s">%d</td>'%("color:#c53030" if row_count>3 else "color:#4a5568",row_count)
            h+='</tr>'
    ...
    # ===================================================================
    # FONCTIONS POUR LA PAGE APPEL ANALYSE BACKLOG
    # ===================================================================
    def filter_ot_by_keyword(df, keyword):
        """Filtre les OT ou toute colonne texte contient le mot-cle (insensible a la casse)."""
        mask = pd.Series(False, index=df.index)
        kw = str(keyword).upper().strip()
    ...
    def html_backlog_pivot(pivot_df, css_class, pct_col_name=None, pct_values=None):
        """
        Construit un tableau HTML depuis un DataFrame pivot.
        pivot_df : index=Poste, columns=catégories, values=nombres
        pct_col_name : nom de la colonne % à ajouter (ex: "% Caractérisé")
        pct_values : liste des valeurs % (même ordre que pivot_df.index)
        """
        if pivot_df.empty:
            return '<div class="es">Aucune donnee</div>'

        cats = list(pivot_df.columns)
        all_cols = ["Poste de travail"] + cats
        if pct_col_name:
            all_cols.append(pct_col_name)
        ...
        grand_totals = {}
        for c in cats:
            grand_totals[c] = int(pivot_df[c].sum()) if c in pivot_df.columns else 0
        ...
        for idx, poste in enumerate(pivot_df.index):
            ...
            if pct_col_name and pct_values and idx < len(pct_values):
                pv = pct_values[idx]
                try:
                    pf = float(str(pv).replace('%','').strip())
                    h += '<td style="text-align:center;%s">%s</td>' % (cs("%.1f" % pf), pv)
                except:
                    h += '<td style="text-align:center">%s</td>' % pv
            h += '<td style="text-align:center;font-weight:800;background:#edf2f7">%d</td>' % row_total
            h += '</tr>'

        # Ligne Total
        h += '<tr class="tr"><td class="poste-cell">Total</td>'
        for c in cats:
            h += '<td style="text-align:center">%d</td>' % grand_totals[c]
        if pct_col_name:
            if pct_values:
        ...
        h += '<td style="text-align:center;font-weight:900;font-size:14px">%d</td>' % grand_total_all
        h += '</tr>'
        return h + '</tbody></table>'
    ...
    def anl_pie_chart(data,names_col,values_col,title,colors=None,min_pct=3.0):
        ...
        if not has_small:
            ...
            fig.update_layout(margin=dict(t=60,b=50,l=20,r=20),height=440,autosize=True,title_font_size=14,
                legend=dict(font_size=11,orientation="h",yanchor="bottom",y=-0.12,title_text="Legende",title_font_size=11))
            return fig
        else:
            others_label="Autres (%d)"%len(small)
            ...
            for i in range(len(main_df)):
                main_colors.append("#CBD5E0" if i==len(main_df)-1 else base_colors[i%len(base_colors)])
            sub_colors=[base_colors[(len(big)+i)%len(base_colors)] for i in range(len(sub_df))]
            fig=make_subplots(rows=1,cols=2,specs=[[{"type":"pie"},{"type":"pie"}]],
                subplot_titles=["<b>%s</b>"%title,"<b>Detail 'Autres' (%d)</b>"%len(small)],horizontal_spacing=0.08)
            ...
            fig.update_layout(margin=dict(t=60,b=50,l=10,r=10),height=440,autosize=True,title_font_size=14,showlegend=True,
                legend=dict(font_size=10,orientation="h",yanchor="bottom",y=-0.06,title_text="Legende",title_font_size=11))
            return fig
    ...
    def make_kpi_bar_chart_with_target(kpi_list,actuals,targets,title,color_ok,color_fail):
        ...
        for k in kpi_list:
            ...
            met=av<=tv if is_lb(k) else av>=tv
            names.append(k); vals.append(round(av,1))
            colors.append(color_ok if met else color_fail); cibles.append(tv)
        fig=go.Figure()
    ...
    with st.sidebar:
        ...
        if show_filters:
            ...
            unf=False; ot_f=None; av_f=None; apm=[]; sp=["All"]; sa=["All"]; sd=["All"]
    ...
    if not unf or (ot_f is not None and av_f is not None):
        try:
            ...
            df_ot=raw_ot[raw_ot["Poste travail princ."].isin(sp_sel)].copy()
            df_av=raw_av[raw_av["Poste travail princ."].isin(sp_sel)].copy()
            ...
            if dr and len(dr)==2:
                ...
                if "Date de début planifiée" in df_ot.columns:
                    df_ot=df_ot[(df_ot["Date de début planifiée"].isna())|(df_ot["Date de début planifiée"]>=d1)&(df_ot["Date de début planifiée"]<=d2)]

            posts=sorted(sp_sel); now=datetime.now()
            kp=calc_kpis(df_ot,df_av,now,posts)
            ckdf=kp['ckdf']; dfp=kp['dfp']; avf=kp['avf']
            ...
            tot_ot=len(dfp); tot_av=len(avf)
            avg_p=np.mean(list(perf_scores.values())) if perf_scores else 0
            avg_q=np.mean(list(qual_scores.values())) if qual_scores else 0
            ...
            top5_perf=pd.DataFrame(perf_ranked[:5],columns=["Poste","Score"])
            bot5_perf=pd.DataFrame(perf_ranked[-5:][::-1],columns=["Poste","Score"]) if len(perf_ranked)>5 else pd.DataFrame(columns=["Poste","Score"])
            qual_ranked=sorted(qual_scores.items(),key=lambda x:x[1],reverse=True)
            top5_qual=pd.DataFrame(qual_ranked[:5],columns=["Poste","Score"])
            bot5_qual=pd.DataFrame(qual_ranked[-5:][::-1],columns=["Poste","Score"]) if len(qual_ranked)>5 else pd.DataFrame(columns=["Poste","Score"])

            ot_statut_df=dfp.groupby("Statut OT",dropna=False).size().reset_index(name="Nombre")
            ot_statut_df["Statut OT"]=ot_statut_df["Statut OT"].fillna("INCONNU")
            avis_statut_df=avf.groupby("Statut utilisateur",dropna=False).size().reset_index(name="Nombre")
            avis_statut_df["Statut utilisateur"]=avis_statut_df["Statut utilisateur"].fillna("INCONNU")
            ...
            for p in posts:
                ...
                for k in QK:
                    ...
                    if not (v<=t if is_lb(k) else v>=t):
                        ano_p_rows.append({"Poste de travail":p,"KPI":k,"Valeur":round(v,1),"Cible":t,"Ecart":round(v-t,1)})
                for k in PK:
                    v=ckdf.loc[p,k]; t=CIBLE.get(k,100)
                    if not (v<=t if is_lb(k) else v>=t):
                        ano_q_rows.append({"Poste de travail":p,"KPI":k,"Valeur":round(v,1),"Cible":t,"Ecart":round(v-t,1)})
            ...
            save_kpis_to_excel(prows,pcols,qrows,qcols,ano_p_excel,ano_p_cols=["Poste de travail","KPI","Valeur","Cible","Ecart"],ano_q_r=ano_q_excel,ano_q_c=["Poste de travail","KPI","Valeur","Cible","Ecart"],sheet_name=fichier_date)

            # ===================================================================
            # DONNEES POUR LA PAGE APPEL ANALYSE BACKLOG
            # ===================================================================

            # 1) OT OMS
            df_oms = filter_ot_by_keyword(dfp, "OMS")
            if not df_oms.empty and "Statut OT" in df_oms.columns:
                oms_pivot = pd.pivot_table(df_oms, index="Poste travail princ.", columns="Statut OT",
                    values="Ordre", aggfunc="count", fill_value=0).reindex(posts, fill_value=0)
                # Garder seulement les postes qui ont des données
                oms_pivot = oms_pivot.loc[oms_pivot.sum(axis=1) > 0]
            else:
                oms_pivot = pd.DataFrame()

            # 2) OT THERMOGRAPHIE
            df_thermo = filter_ot_by_keyword(dfp, "THERMOGRAPHIE")
            if not df_thermo.empty and "Statut OT" in df_thermo.columns:
                thermo_pivot = pd.pivot_table(df_thermo, index="Poste travail princ.", columns="Statut OT",
                    values="Ordre", aggfunc="count", fill_value=0).reindex(posts, fill_value=0)
                thermo_pivot = thermo_pivot.loc[thermo_pivot.sum(axis=1) > 0]
            else:
                thermo_pivot = pd.DataFrame()

            # 3) Backlog Préparation (Statut OT == CRÉÉ, Caractérisé vs Non)
            df_cre = dfp[dfp["Statut OT"] == "CRÉÉ"] if "Statut OT" in dfp.columns else pd.DataFrame()
            if not df_cre.empty:
                bp_pivot = pd.pivot_table(df_cre, index="Poste travail princ.", columns="Backlog preparation",
                    values="Ordre", aggfunc="count", fill_value=0).reindex(posts, fill_value=0)
                for c in ["CARACTERISE", "NON CARACTERISE"]: bp_pivot[c] = bp_pivot.get(c, 0)
                bp_pivot["Total"] = bp_pivot["CARACTERISE"] + bp_pivot["NON CARACTERISE"]
                bp_pivot = bp_pivot.loc[bp_pivot["Total"] > 0]
                bp_pct = []
                for p in bp_pivot.index:
                    t = bp_pivot.loc[p, "Total"]
                    bp_pct.append(round(bp_pivot.loc[p, "CARACTERISE"] / t * 100, 1) if t > 0 else "0.0%")
            else:
                bp_pivot = pd.DataFrame(); bp_pct = []

            # 4) Backlog Exécution (Statut OT == LANC avec SOPL, par age d'exécution)
            df_exec = dfp[(dfp["Statut OT"] == "LANC") & (dfp["Contient SOPL"] == 1)] if "Statut OT" in dfp.columns else pd.DataFrame()
            if not df_exec.empty and "aex" in df_exec.columns:
                be_pivot = pd.pivot_table(df_exec, index="Poste travail princ.", columns="aex",
                    values="Ordre", aggfunc="count", fill_value=0).reindex(posts, fill_value=0)
                for c in ["<1 mois", ">3 mois", "1 mois < <3 mois"]: be_pivot[c] = be_pivot.get(c, 0)
                be_pivot["Total"] = be_pivot[["<1 mois", "1 mois < <3 mois", ">3 mois"]].sum(axis=1)
                be_pivot = be_pivot.loc[be_pivot["Total"] > 0]
            else:
                be_pivot = pd.DataFrame()
            ...
            st.markdown('<div class="cr">'
                '<div class="cc c1"><div class="cv">%d</div><div class="cl">OT Total</div></div>'
                '<div class="cc c2"><div class="cv">%d</div><div class="cl">Avis Sans OT</div></div>'
                ...
                '</div>'%(tot_ot,tot_av,avg_p,avg_q),unsafe_allow_html=True)

            tab_synthese, tab_performance, tab_qualite, tab_anomalies, tab_backlog, tab_journal, tab_classement = st.tabs([
                "📋 Synthese & Actions","⚡ Performance","🎯 Qualite","⚠️ Anomalies","📦 Appel Analyse Backlog","📖 Journal","🏆 Classement"
            ])

            # ============ ONGLET 1 : SYNTHESE & ACTIONS ============
            with tab_synthese:
                st.markdown('<div class="stl s">Synthese & Actions — Vue globale</div>',unsafe_allow_html=True)
                st.markdown('<div class="stl q">Repartition par Statut</div>',unsafe_allow_html=True)
                ...
                with col_p1:
                    ...
                    else: st.markdown('<div class="es">Aucune donnee OT</div>',unsafe_allow_html=True)
                with col_p2:
                    ...
                    else: st.markdown('<div class="es">Aucune donnee Avis</div>',unsafe_allow_html=True)

                st.markdown('<div class="stl p">Performance — avec Objectifs</div>',unsafe_allow_html=True)
                fig_perf_bar=make_kpi_bar_chart_with_target(QK,global_actuals,CIBLE,"Performance — Valeurs vs Cibles","#38a169","#e53e3e")
                if fig_perf_bar: st.plotly_chart(fig_perf_bar,use_container_width=True)

                st.markdown('<div class="stl q">Qualite — avec Objectifs</div>',unsafe_allow_html=True)
                fig_qual_bar=make_kpi_bar_chart_with_target(PK,global_actuals,CIBLE,"Qualite — Valeurs vs Cibles","#3182ce","#e53e3e")
                if fig_qual_bar: st.plotly_chart(fig_qual_bar,use_container_width=True)

                st.markdown('<div class="stl p">Classement Performance — Top 5 / Bottom 5</div>',unsafe_allow_html=True)
                fig_tp=make_rank_bar_chart(top5_perf,bot5_perf,"Top 5 / Bottom 5 — Score Performance","#38a169","#e53e3e")
                if fig_tp: st.plotly_chart(fig_tp,use_container_width=True)

                st.markdown('<div class="stl q">Classement Qualite — Top 5 / Bottom 5</div>',unsafe_allow_html=True)
                fig_tq=make_rank_bar_chart(top5_qual,bot5_qual,"Top 5 / Bottom 5 — Score Qualite","#3182ce","#e53e3e")
                if fig_tq: st.plotly_chart(fig_tq,use_container_width=True)

                st.markdown('<div class="stl a">Actions Recommandees</div>',unsafe_allow_html=True)
                ...
                st.markdown('<div class="stl c">Comparaison Performance vs Qualite par Poste</div>',unsafe_allow_html=True)
                st.markdown(html_grouped_bars(posts,perf_scores,qual_scores,"Score Performance vs Qualite par Poste"),unsafe_allow_html=True)
            ...
            with tab_performance:
                ...
                with st.expander("📊 Graphique detaille"):
                    fig_p2=make_kpi_bar_chart_with_target(QK,global_actuals,CIBLE,"Performance — Detail","#38a169","#e53e3e")
                    if fig_p2: st.plotly_chart(fig_p2,use_container_width=True)
            ...
            with tab_qualite:
                ...
                with st.expander("📊 Graphique detaille"):
                    fig_q2=make_kpi_bar_chart_with_target(PK,global_actuals,CIBLE,"Qualite — Detail","#3182ce","#e53e3e")
                    if fig_q2: st.plotly_chart(fig_q2,use_container_width=True)

            # ============ ONGLET 4 : ANOMALIES ============
            with tab_anomalies:
                st.markdown('<div class="stl a">Anomalies Detectees — Matrice KPI x Poste</div>',unsafe_allow_html=True)
                if tot_ano_p==0 and tot_ano_q==0:
                    st.markdown('<div class="es" style="padding:30px;font-size:16px">✅ Aucune anomalie — Tous les indicateurs atteints</div>',unsafe_allow_html=True)
                else:
                    if tot_ano_p > 0:
            ...
            # ================================================================
            # ============ ONGLET 5 : APPEL ANALYSE BACKLOG ================
            # ================================================================
            with tab_backlog:

                # ---- SECTION 1 : OT OMS ----
                st.markdown('<div class="stl om">🔍 Analyse OT — OMS</div>',unsafe_allow_html=True)
                if oms_pivot.empty:
                    st.markdown('<div class="es">Aucun OT contenant "OMS" trouve dans les donnees.</div>',unsafe_allow_html=True)
                else:
                    nb_oms = int(oms_pivot.sum().sum())
                    st.markdown('<div style="font-size:12px;color:#718096;margin-bottom:4px">%d OT trouves avec le mot-cle "OMS" — Repartition par Poste et Statut</div>'%nb_oms,unsafe_allow_html=True)
                    col_oms1, col_oms2 = st.columns([3, 2])
                    with col_oms1:
                        st.markdown(html_backlog_pivot(oms_pivot, "bt-oms"),unsafe_allow_html=True)
                    with col_oms2:
                        oms_pie_data = oms_pivot.sum().reset_index()
                        oms_pie_data.columns = ["Statut OT", "Nombre"]
                        fig_oms = anl_pie_chart(oms_pie_data, "Statut OT", "Nombre",
                            "Repartition OMS par Statut",
                            colors=["#38a169","#3182ce","#d69e2e","#e53e3e","#805ad5","#CBD5E0"])
                        if fig_oms: st.plotly_chart(fig_oms,use_container_width=True)
                        else: st.markdown('<div class="es">Pas de donnee pour le pie</div>',unsafe_allow_html=True)

                st.markdown('<div class="sec-sep"></div>',unsafe_allow_html=True)

                # ---- SECTION 2 : OT THERMOGRAPHIE ----
                st.markdown('<div class="stl th">🌡️ Analyse OT — THERMOGRAPHIE</div>',unsafe_allow_html=True)
                if thermo_pivot.empty:
                    st.markdown('<div class="es">Aucun OT contenant "THERMOGRAPHIE" trouve dans les donnees.</div>',unsafe_allow_html=True)
                else:
                    nb_thermo = int(thermo_pivot.sum().sum())
                    st.markdown('<div style="font-size:12px;color:#718096;margin-bottom:4px">%d OT trouves avec le mot-cle "THERMOGRAPHIE" — Repartition par Poste et Statut</div>'%nb_thermo,unsafe_allow_html=True)
                    col_th1, col_th2 = st.columns([3, 2])
                    with col_th1:
                        st.markdown(html_backlog_pivot(thermo_pivot, "bt-th"),unsafe_allow_html=True)
                    with col_th2:
                        thermo_pie_data = thermo_pivot.sum().reset_index()
                        thermo_pie_data.columns = ["Statut OT", "Nombre"]
                        fig_th = anl_pie_chart(thermo_pie_data, "Statut OT", "Nombre",
                            "Repartition THERMOGRAPHIE par Statut",
                            colors=["#3182ce","#38a169","#d69e2e","#e53e3e","#805ad5","#CBD5E0"])
                        if fig_th: st.plotly_chart(fig_th,use_container_width=True)
                        else: st.markdown('<div class="es">Pas de donnee pour le pie</div>',unsafe_allow_html=True)

                st.markdown('<div class="sec-sep"></div>',unsafe_allow_html=True)

                # ---- SECTION 3 : BACKLOG PREPARATION ----
                st.markdown('<div class="stl bp">📋 Backlog Preparation par Poste de Travail</div>',unsafe_allow_html=True)
                if bp_pivot.empty:
                    st.markdown('<div class="es">Aucun OT en statut CRÉÉ (backlog preparation vide).</div>',unsafe_allow_html=True)
                else:
                    nb_bp = int(bp_pivot["Total"].sum())
                    st.markdown('<div style="font-size:12px;color:#718096;margin-bottom:4px">%d OT en attente de preparation — Caracterisation du backlog</div>'%nb_bp,unsafe_allow_html=True)
                    col_bp1, col_bp2 = st.columns([3, 2])
                    with col_bp1:
                        # Retirer la colonne Total du pivot pour le HTML (elle est ajoutée automatiquement)
                        bp_show = bp_pivot[["CARACTERISE", "NON CARACTERISE"]].copy()
                        st.markdown(html_backlog_pivot(bp_show, "bt-bp", "% Caractérisé", bp_pct),unsafe_allow_html=True)
                    with col_bp2:
                        bp_pie_data = pd.DataFrame({
                            "Statut": ["CARACTERISE", "NON CARACTERISE"],
                            "Nombre": [int(bp_pivot["CARACTERISE"].sum()), int(bp_pivot["NON CARACTERISE"].sum())]
                        })
                        fig_bp = anl_pie_chart(bp_pie_data, "Statut", "Nombre",
                            "Backlog Preparation — % Caracterise",
                            colors=["#38a169","#e53e3e"])
                        if fig_bp: st.plotly_chart(fig_bp,use_container_width=True)
                        else: st.markdown('<div class="es">Pas de donnee</div>',unsafe_allow_html=True)

                st.markdown('<div class="sec-sep"></div>',unsafe_allow_html=True)

                # ---- SECTION 4 : BACKLOG EXECUTION ----
                st.markdown('<div class="stl be">⚙️ Backlog Execution par Poste de Travail</div>',unsafe_allow_html=True)
                if be_pivot.empty:
                    st.markdown('<div class="es">Aucun OT en execution (backlog execution vide).</div>',unsafe_allow_html=True)
                else:
                    nb_be = int(be_pivot["Total"].sum())
                    st.markdown('<div style="font-size:12px;color:#718096;margin-bottom:4px">%d OT en cours d\'execution — Repartition par age d\'execution</div>'%nb_be,unsafe_allow_html=True)
                    col_be1, col_be2 = st.columns([3, 2])
                    with col_be1:
                        be_show = be_pivot[["<1 mois", "1 mois < <3 mois", ">3 mois"]].copy()
                        st.markdown(html_backlog_pivot(be_show, "bt-be"),unsafe_allow_html=True)
                    with col_be2:
                        be_pie_data = pd.DataFrame({
                            "Age Execution": ["<1 mois", "1 mois < <3 mois", ">3 mois"],
                            "Nombre": [
                                int(be_pivot["<1 mois"].sum()),
                                int(be_pivot["1 mois < <3 mois"].sum()),
                                int(be_pivot[">3 mois"].sum())
                            ]
                        })
                        fig_be = anl_pie_chart(be_pie_data, "Age Execution", "Nombre",
                            "Backlog Execution — Repartition par Age",
                            colors=["#38a169","#d69e2e","#e53e3e"])
                        if fig_be: st.plotly_chart(fig_be,use_container_width=True)
                        else: st.markdown('<div class="es">Pas de donnee</div>',unsafe_allow_html=True)
            ...
            with tab_journal:
                ...
                if hist_df.empty:
                    st.markdown('<div class="es">Pas assez d\'historique (minimum 2 periodes)</div>',unsafe_allow_html=True)
                else:
                    var_df=calculate_variations(hist_df)
                    if var_df.empty:
                        st.markdown('<div class="es">Aucune variation calculee</div>',unsafe_allow_html=True)
                    else:
                        ...
                        if jrn.empty:
                            ...
                            with st.expander("📥 Export"):
                                export_btn(jrn.drop(columns=["Significatif"],errors="ignore"),"journal_variations.xlsx")

            # ============ ONGLET 7 : CLASSEMENT ============
            with tab_classement:
                st.markdown('<div class="stl c">Classement Global des Postes</div>',unsafe_allow_html=True)
                ...
                if not hist_df.empty:
                    ...
                    if not var_df.empty:
                        ...
                        st.markdown('<div class="stl s">Evolution — Top / Bottom Historique</div>',unsafe_allow_html=True)
        ...
        if unf:
            st.markdown("""<div style="text-align:center;padding:60px;color:#718096"><div style="font-size:64px;margin-bottom:16px">📁</div><h2 style="color:#1e3a5f">Chargement de fichiers</h2><p>Veuillez charger les fichiers OT et AVIS dans la barre laterale.</p></div>""",unsafe_allow_html=True)
        else:
